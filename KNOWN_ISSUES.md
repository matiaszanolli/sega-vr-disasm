...
| 60 FPS via pipeline-overlap of `sh2_send_cmd` (Phase 6) for racing | Racing-isolated profiling (`VRD_SCENE=0x4CBC`): the 68K is NOT blocked on the Master SH2 during racing — the `sh2_send_cmd` sync-wait is negligible and the 68K is **63% V-blank idle**. The ~13% wait seen earlier was a **car-select/attract artifact** of mixed-scene profiling. The 20 FPS cap is the **state machine** (1 state/V-INT), so the lever is **Phase 7 (per-TV-frame logic)**, not Phase 6. See VR60_ROADMAP §1.1b / §11. |
| Unbounded retry loop for cmd $3E/$3F ACK (`.retrigger: ... beq.s .retrigger`, force 0→1 transition on COMM0_HI) in `vr60_entity_transfer.asm`/`vr60_ai_entity_transfer.asm`/`vr60_globals_transfer.asm`/`vr60_comm_trigger.asm` | Headlessly "verified working" (DREQ_LEN drained, sentinel fired, fb_crc unchanged) but the verification never actually exercised real GP racing — see next entry. Real GP racing hard-hung the 68K (black screen, audio still running). These functions are shared with the always-active 2P path (`state4_epilogue`), so the change broke already-working functionality. Fully reverted to `HEAD` 2026-07-13. If retried: hard-cap the retry count, and implement in 1P-exclusive copies rather than editing the shared functions. See `analysis/VR60_PHASE1_CMD3E_ACK_HANG.md` §13. |
| Using `--autoplay` to headlessly test 1-player GP racing | `profiling_frontend`'s `--autoplay` (press START blindly every 90 frames, then hold A after frame 1200) reliably parks the game in scene `$5586` (Free Run/TT, `state_disp_005586`) and never reaches scene `$4CBC` (real GP racing, `state_disp_004cb8`) — confirmed out to 3600 frames, and even with no input at all. The `[racing]` label in the progress output is a naive frame-count heuristic, not derived from game state — don't trust it. Confirmed 2026-07-13 by watching `$FF0004` directly. Use `VRD_LOAD_STATE=path` (added this session) with a manually-captured GP-racing savestate instead. |
