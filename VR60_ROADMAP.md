...
two-byte pair delta. Q-027 remains non-promotable. Continue one independent stage at a time:
separately prove a bridge into the renderer-consumed C128/C178/C254 descriptors, and only then
make subsystem authority/bypass changes.
...
Q-023's mailbox correction and Q-027's bounded cmd `$3F` validation gate are complete. The
immediate stage is reversible A/B/C probing of the C128/C178/C254 descriptor families consumed
by cmd `$02`. Do not combine that bridge probe with authority transfer or cadence work.
