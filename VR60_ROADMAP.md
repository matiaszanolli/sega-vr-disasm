...
remains unreachable in both arms. Q-027 now has a post-approval candidate archive for one bounded
...
DRC arm has full displayed-CRC equality. Q-027 is non-promotable and still awaits completed-diff
audit. Continue one independent stage at a time: audit Q-027, then separately prove a bridge into
the renderer-consumed C128/C178/C254 descriptors, and only then subsystem authority/bypass changes.
...
Q-023's mailbox correction is complete and Q-027 now has a bounded cmd `$3F` candidate archive.
The immediate stage is fresh completed-diff audit of Q-027. Do not begin renderer bridging,
authority transfer, or cadence work before that review accepts the candidate.
...
| Q-021 | Are AI transfer mode 2 and cmd `$3F` safe in 1P? | Phases 1, 3, 4 | **PARTIALLY RESOLVED: MODE-2 FIRST-BOUNDARY PASS; BOUNDED Q-027 CANDIDATE** | The isolated mode-2 CMDINT/DREQ pair proves exact 3,840-byte source/FIFO/destination equality at the first post-transfer boundary (`96d79e3f…ee276` / `da5ce4ec…309b8`). Mode 2 remains unpromoted and disabled in the default; later destination mutation prevents persistent ownership or consumer claims. Q-027 now covers only one player-only COMM0/stock-table `$3F` transaction and awaits completed-diff audit; the full AI/shared-lane/renderer pipeline remains open. |
...
| Q-027 | Can the smallest hardware-safe cmd `$3F` gate reach the real stock table using only Master-owned COMM0? | Phase 3 shadow transport | **CANDIDATE PASS — COMPLETED-DIFF AUDIT PENDING; NON-PROMOTABLE** | ACTIVE `50c20e1a…34097` and CONTROL `33bfdd8c…e511` preserve all accepted identities and differ only at file `$3041A2-$3041A3`. The exact two-edge park/normalize/publish protocol never publishes COMM1-7. Two fresh interpreter repeats per arm pin `T_I=1264`, one CRC mismatch at `T_I+2` (`344A67A6/D08BAC75`), CRC convergence at `T_I+3`, full-profile convergence at `T_I+6`, exact stock table `$02301500`, Q-026-identical player result, fixed mailbox/context/stack/write bounds, and exact swap chronology. Two normal-DRC repeats per arm pin `T_D=1262`, zero CRC mismatch, and full-profile equality from `T_D+1`. This proves no legacy shared-lane transport, renderer equivalence/bridge, collision, authority, cadence, real-hardware, budget, or FPS property. |
...
| 2026-08-13 | Accept the approved Q-027 v5 design as a completed-diff candidate, pending final audit | The ROM pair is byte-unchanged from approved v4. Fresh interpreter and DRC captures reproduce every predeclared transaction-relative mismatch and convergence value twice per arm; all protocol, context, write, display/swap, tool, and diagnostic identities are fail-closed. | Hide or threshold the interpreter CRC mismatch, invent a timing-matched delay control, promote cmd `$3F`, publish legacy COMM parameters, claim render/cadence/FPS equivalence, or begin the bridge before completed-diff audit. |
