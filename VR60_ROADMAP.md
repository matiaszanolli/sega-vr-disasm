...
remains unreachable in both arms. Continue one independent stage at a time: use the corrected
artifact for observable cmd `$3F` shadow execution, then prove a bridge into the renderer-consumed
...
The next independent stage is Q-023's cmd `$3F` mailbox correction, then cmd `$3F` observable
shadow execution with the 68000 authoritative. Do not combine those two gates with renderer
bridging, authority transfer, or cadence work.
...
| Q-021 | Are AI transfer mode 2 and cmd `$3F` safe in 1P? | Phases 1, 3, 4 | **PARTIALLY RESOLVED: MODE-2 FIRST-BOUNDARY PASS; CMD `$3F` OPEN** | The isolated mode-2 CMDINT/DREQ pair proves exact 3,840-byte source/FIFO/destination equality at the first post-transfer boundary (`96d79e3f…ee276` / `da5ce4ec…309b8`). Mode 2 remains unpromoted and disabled in the default; later destination mutation prevents persistent-ownership or consumer claims. Cmd `$3F` is still disabled/unverified and must first receive Q-023's mailbox correction, then run only as observable shadow computation with the 68000 authoritative. |
...
| Q-023 | Is cmd `$3F`'s COMM mailbox actually in SDRAM? | Phase 1B/3 | **RESOLVED: CORRECTED STATIC ARTIFACT; NOT EXECUTED** | `$2200BC00` is the cache-through cartridge-ROM alias. Source-built ACTIVE `8709aed4…5a59ef` selects `$2600BC00`; STAGE-CONTROL is the exact default `6f2768f2…2523900`, with the sole ROM delta at file `$301638`. Opcode `$D449`, its sole literal ownership, handler size, jump entry, and accepted mode-1/mode-2 identities are pinned. Cmd `$3F` remains disabled, so runtime mailbox/COMM/shadow safety is the next gate. |
...
| R-029 | A static corrected cmd `$3F` literal could be mistaken for runtime protocol validation | Critical | Q-023 / cmd `$3F` shadow | **OPEN; EXCLUDED FROM Q-023** | The Q-023 pair keeps cmd `$3F` unreachable and proves only source-built byte isolation. The next gate must execute the corrected arm as observable shadow work and separately prove COMM read/write exclusion, cache-through accesses, write flushes, completion, Slave retrigger safety, and lifecycle liveness before any promotion. |
