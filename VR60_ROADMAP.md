...
into the ordinary default. A separate Q-021 mode-2 ACTIVE/STAGE-CONTROL pair now also has an
approved isolated PicoDrive validation-stage result. It proves exact 3,840-byte AI staging and
DREQ transport at the first post-transfer boundary with 1,920 FIFO words, 480 FULL-checked
four-word groups, zero COMM access, and unchanged default/mode-1 identities. Mode 2 remains
disabled/unreachable in the ordinary default. Its `$06010000-$06010EFF` destination is approved
only at that observed boundary: the Slave stack grows downward below `$06010000`, but an
unidentified later writer mutates the range, so persistent ownership, consumer use, promotion,
and later-frame safety remain unproven. Cmd `$3F` and the 68000 physics bypass are disabled.
Therefore
...
VR60-008 remains blocked until that unchanged control passes. Q-020 modes 0 and 1 and Q-021
mode 2 have now passed their separately scoped gates. Q-023 now also has a source-built,
static-only ACTIVE/STAGE-CONTROL result: ACTIVE changes only cmd `$3F`'s file-$301638 mailbox
byte (`$2200BC00 -> $2600BC00`), while CONTROL remains the exact ordinary default and cmd `$3F`
remains unreachable in both arms. Q-027 now has an approved archive for one bounded
COMM0-only stock-table `$3F` player-shadow invocation. Its exact interpreter observation retains
one displayed CRC mismatch at `T_I+2` and proves deterministic convergence; its separate normal
DRC arm has full displayed-CRC equality. A fresh completed-diff audit reproduced both build
targets, all 23 test methods, the eight-run capture contract, exact identities, and the sole
two-byte pair delta. Q-027 remains non-promotable.
...
accepted-format artifacts and invoking the production validator.
...
The cmd `$3E` mode-0 sub-gate and isolated mode-1/mode-2 validation stages are complete. The
next issue is the cmd `$3F` mailbox correction, followed separately by cmd `$3F` shadow
execution. Each renderer/authority transition remains a separate issue with its own sentinel and
control comparison.
...
claim. At that point the next independent stage was cmd `$3E` AI transfer mode 2; it was not
combined with cmd `$3F`.

### Q-021 mode-2 validation-stage acceptance (2026-08-12)

The isolated cmd `$3E` mode-2 ACTIVE/STAGE-CONTROL pair is **APPROVED strictly as a first-
post-transfer PicoDrive validation-stage result**. ACTIVE is `96d79e3f…ee276`, STAGE-CONTROL is
`da5ce4ec…309b8`, and the 1,056-byte Master ISR is `b7fc5726…aaaa7`. The ordinary default remains
`6f2768f2…2523900`, and the accepted mode-1 ACTIVE/CONTROL/ISR identities remain byte-exact.

Both arms stage the authoritative 15-record AI table from `$FF9100-$FF9FFF` at
`$FF6B40-$FF7A3F`. ACTIVE alone uses two Master CMD edges and zero COMM0-COMM7 accesses to move
3,840 bytes into physical SDRAM `$06010000-$06010EFF`: 1,920 FIFO words in 480 FULL-checked
groups of four. Two normal repeats prove full source/FIFO/destination equality at frame 1260,
terminal DAR `$06010F00`, TCR0 zero, TE read-1/write-0 acknowledgement, exact CMD clear/readback,
and setup/completion counts `1/1`; both controls stage identical bytes and record zero transport.
Two repeats per arm on the seeded name/replay route correctly reach stock replay `$00885618`
and record zero racing-hook transport. Only the predeclared frame-1241 stock-safe VRES boundary
is accepted.

A fresh Auditor reproduced the clean build, the complete nested mode-1/mode-2 gates, exact
hashes, 14 focused adversarial tests, raw evidence binding, literal ownership, and clean diff.
The Auditor approved `$06010000` only at the captured boundary: stock Slave R15 is exactly there,
stock/ISR stacks grow downward below it, the payload grows upward, and both ACTIVE repeats prove
physical equality before later reuse. Exploratory later execution mutates the destination through
an unidentified writer. Consequently this result does not establish persistent allocation,
later-frame ownership, a consumer, promotion, organic name-entry gameplay, arbitrary reset, real
hardware, authority transfer, cadence, FPS, or CPU budget. Evidence:
`analysis/evidence/vr60-q021-mode2-cmdint-gate/`.

Q-023's mailbox correction and Q-027's bounded cmd `$3F` validation gate are complete. Q-028 is
the active blocked gate under the exact rollback above. All earlier Q-028 observations are
diagnostic only. The immediate stage is the audited v3-v7 tooling repair, prospective
engine-specific schedule freeze, and fresh complete two-engine recapture; later renderer
discovery, bridge implementation, authority transfer, and cadence work remain behind it.
...
**Practical upshot at the time (mode 2 was later bounded by the 2026-08-12 gate above)**: AI
transfer (cmd `$3E` mode 2) and cmd `$3F` are neither proven safe nor
...
| Intended SDRAM mailbox | $0600BC00 | 16 bytes | Q-023 source-built ACTIVE selects cache-through `$2600BC00`; ordinary default retains legacy `$2200BC00` while cmd `$3F` remains disabled |
...
| Q-023 mailbox literal | Source-built ACTIVE/STAGE-CONTROL pair | ACTIVE `$2600BC00`; CONTROL `$2200BC00`; sole ROM delta file `$301638`; static only |
...
- [x] Produce a source-built handler using `$2600BC00` without changing the accepted ordinary default; Q-023 static gate complete
...
| Q-020 | Are cmd `$3E` modes 0/1 correct over a trustworthy 1P run? | Phase 1 | **RESOLVED WITH SEPARATELY SCOPED PASSES** | Mode 0 passed 12 fresh lifecycle runs and is the ordinary default (`6f2768f2…2523900`). The isolated mode-1 CMDINT pair passed canonical Gate B, exact 64B/FULL/TE/re-arm transport, eight normal/seeded-name route captures, and safe-boundary VRES (`96365860…ff4470` / `39177456…34e9fd`). Mode 1 remains unpromoted; name evidence is seeded/replay-only, arbitrary busy-Slave VRES and real hardware are unproven. |
| Q-021 | Are AI transfer mode 2 and cmd `$3F` safe in 1P? | Phases 1, 3, 4 | **PARTIALLY RESOLVED: MODE-2 FIRST-BOUNDARY PASS; BOUNDED Q-027 PASS** | The isolated mode-2 CMDINT/DREQ pair proves exact 3,840-byte source/FIFO/destination equality at the first post-transfer boundary (`96d79e3f…ee276` / `da5ce4ec…309b8`). Mode 2 remains unpromoted and disabled in the default; later destination mutation prevents persistent ownership or consumer claims. Q-027 covers only one player-only COMM0/stock-table `$3F` transaction; the full AI/shared-lane/renderer pipeline remains open. |
| Q-022 | Which racing descriptor block should carry the SH2-authoritative car state? | Phase 5F | **OPEN; Q-028 EVIDENCE RETRACTED** | Static decode proves cmd `$02` reads C128/C178/C254, not C218. Prior Q-028 family observations are diagnostic only and establish no accepted family result. |
| Q-023 | Is cmd `$3F`'s COMM mailbox actually in SDRAM? | Phase 1B/3 | **RESOLVED: CORRECTED STATIC ARTIFACT; Q-027 BOUNDED RUNTIME PASS** | `$2200BC00` is the cache-through cartridge-ROM alias. Source-built ACTIVE `8709aed4…5a59ef` selects `$2600BC00`; STAGE-CONTROL is the exact default `6f2768f2…2523900`, with the sole ROM delta at file `$301638`. Opcode `$D449`, its sole literal ownership, handler size, jump entry, and accepted mode-1/mode-2 identities are pinned. Q-027 separately proves its fixed mailbox only in a player-only validation transaction; full shared-lane semantics remain open. |
...
| Q-026 | Can one bounded player-physics sequence execute safely through direct Master CMDINT before cmd `$3F` shadow integration? | Phase 3 precursor | **RESOLVED — ISOLATED VALIDATION-STAGE PASS; NON-PROMOTABLE** | ACTIVE `c9358ad4…b76f8` and CONTROL `240dfd0a…a66d` differ only at the exact eight-byte CMD edge. Two trustworthy normal-1P repeats per arm prove one accepted 320-byte seed, one ACTIVE entry/completion and zero CONTROL work, allowed-only writes, preserved `+$CE/+$D2/+$D6/+$DA`, admission-local canary integrity, unchanged COMM1/2/7 with COMM2_HI zero at the exact edge, and exact continuing pair/repeat state/caller/framebuffer/dual-SH2 evidence. This is direct invocation only: cmd `$3F`, collision/equivalence, bridge, authority, cadence, and FPS remain open. |
| Q-027 | Can the smallest hardware-safe cmd `$3F` gate reach the real stock table using only Master-owned COMM0? | Phase 3 shadow transport | **RESOLVED — ISOLATED VALIDATION-STAGE PASS; NON-PROMOTABLE** | ACTIVE `50c20e1a…34097` and CONTROL `33bfdd8c…e511` preserve all accepted identities and differ only at file `$3041A2-$3041A3`. The exact two-edge park/normalize/publish protocol never publishes COMM1-7. Two fresh interpreter repeats per arm pin `T_I=1264`, one CRC mismatch at `T_I+2` (`344A67A6/D08BAC75`), CRC convergence at `T_I+3`, full-profile convergence at `T_I+6`, exact stock table `$02301500`, Q-026-identical player result, fixed mailbox/context/stack/write bounds, and exact swap chronology. Two normal-DRC repeats per arm pin `T_D=1262`, zero CRC mismatch, and full-profile equality from `T_D+1`. A fresh completed-diff audit reproduced both builds, all 23 tests, exact identities, and the eight-run archive. This proves no legacy shared-lane transport, renderer equivalence/bridge, collision, authority, cadence, real-hardware, budget, or FPS property. |
| Q-028 | Does exactly one cmd `$02` descriptor family A/B/C represent the complete player car? | Phase 5F renderer discovery | **BLOCKED / INCONCLUSIVE_ENGINE_SCHEDULE; EXACT INDEX FORBIDDEN** | Prior interpreter and DRC-A outputs are diagnostic/ineligible. B/C normal-DRC evidence is absent. Requires the separately audited immutable schedule, all 40 fresh packages, complete raw archives, production mutation pass, and fresh completed-diff audit. |
...
| 2026-08-12 | Accept cmd `$3E` mode 2 strictly as an isolated first-post-transfer validation-stage pass | Two ACTIVE repeats prove exact 3,840-byte source/FIFO/physical-destination equality, 1,920 words, 480 FULL-checked groups, terminal DMAC/TE/CMD chronology, stock-stack separation at `$06010000`, and zero CONTROL/name-route transport. The complete mode-1 gate, default hash, composition, and 14 adversarial tests remain exact under fresh audit. | Promote mode 2, treat `$06010000-$06010EFF` as persistent/unaliased, infer a consumer from later mutation, claim organic name-entry/arbitrary-reset/real-hardware safety, or combine Q-023 mailbox correction with cmd `$3F` shadow/authority/cadence work. |
| 2026-08-12 | Accept Q-023 only as a source-built inactive mailbox-literal correction | ACTIVE `8709aed4…5a59ef` differs from exact-default CONTROL `6f2768f2…2523900` only at file `$301638` (`$22->$26`), selecting cache-through SDRAM `$2600BC00`. The 428-byte handler, `$D449` load and sole literal owner, cmd `$3F` jump, and accepted mode-1/mode-2 identities are fail-closed. | Change the ordinary accepted default, enable or execute cmd `$3F`, infer runtime mailbox/COMM safety, or combine shadow/bridge/authority/cadence work. |
| 2026-08-12 | Accept Q-026 only as a bounded direct player-physics CMDINT precursor | The exact pair and four-run archive prove one direct invocation with full ISR/stack/write/field/COMM/liveness bounds while all accepted identities and the authoritative 68000 path remain intact. The failed startup-canary pilot is retained and the admission-local refresh is independently verified. | Promote Q-026, describe it as cmd `$3F` shadow execution, infer collision/equivalence or renderer consumption, transfer authority, change cadence, publish FPS/CPU claims, or generalize exact-edge COMM2_HI idleness to unrelated renderer traffic. |
| 2026-08-13 | Accept Q-027 only as an isolated bounded stock-table `$3F` validation-stage pass | The ROM pair is byte-unchanged from approved v4. Fresh interpreter and DRC captures reproduce every predeclared transaction-relative mismatch and convergence value twice per arm; all protocol, context, write, display/swap, tool, and diagnostic identities are fail-closed. A fresh completed-diff audit reproduced both builds, all 23 tests, exact identities, and the eight-run archive. | Hide or threshold the interpreter CRC mismatch, invent a timing-matched delay control, promote cmd `$3F`, publish legacy COMM parameters, claim render/cadence/FPS equivalence, or combine the next bridge probe with authority/cadence work. |
| 2026-08-13 | Retract Q-028 `INCONCLUSIVE_COMPOSITE`; restore `BLOCKED / INCONCLUSIVE_ENGINE_SCHEDULE` | Interpreter-only completion did not satisfy the approved two-engine requirement; B/C normal-DRC arms are absent, raw retention and applied-input comparison were incomplete, and the owner/mutation claims were unsound. | Reuse any old capture/hash, infer one engine's schedule from another, accept supplied equality booleans, or begin exact-index/bridge/authority/cadence work before the fresh audited v3-v7 composition. |
...
| R-028 | Mode-2 destination `$06010000-$06010EFF` is mutated after the accepted first post-transfer boundary by an unidentified writer | Critical | Q-021 mode 2 / later authority work | **OPEN; BOUNDED OUT OF ISOLATED TRANSPORT PASS** | Frame-1260 evidence proves exact physical transport while Slave R15 remains `$06010000` and both stacks grow below the range. Later mutation prevents any claim that the destination is persistent, unaliased, or consumed by the intended SH2 path. Identify every later writer/consumer and prove ownership before promotion, comparison, or authority transfer; never extend the first-boundary result to later frames. |
| R-029 | A bounded cmd `$3F` shadow invocation could be mistaken for full shared-lane or renderer validation | Critical | Q-023/Q-027 / cmd `$3F` shadow | **MITIGATED FOR THE Q-027 PASS; OPEN FOR LATER STAGES** | Q-027 executes only the specialized player-only path through stock table `$3F`, publishes no COMM1-7, and writes a fixed proof mailbox. Its result explicitly retains one interpreter CRC mismatch and is non-promotable. Legacy parameters, full pipeline, renderer consumption/equivalence, bridge, collision, authority, cadence, and real hardware remain independent gates. |
| R-030 | A startup-written dedicated-stack canary may be overwritten before the racing CMD edge | Critical | Q-026 / later CMDINT work | **MITIGATED FOR Q-026; GENERAL HAZARD** | The first Q-026 pilot failed closed with `$2600FB90=0` despite successful boot initialization. The approved ISR now writes and immediately reads `$5132534B` at CMD admission before the first dedicated-stack write and checks it again after return. Retain the failed pilot; future interrupt stages must establish canary validity at their own admission boundary rather than infer startup persistence. |
| R-031 | Real ACTIVE/CONTROL Master work can shift sampled render scheduling without a Q-027 framebuffer write | High | Q-027 observer acceptance | **BOUNDED FOR THE ISOLATED Q-027 PASS ONLY** | The exact interpreter mismatch is fixed at `T_I+2`, with exact values and uninterrupted CRC/full-profile convergence; normal DRC has no CRC mismatch. Every raw delta repeats exactly and swap/watch/write chronology is equal. This is not causal arbitration proof or permission for a later render-equivalence exception. |
| R-032 | A diagnostic visible renderer component can be mistaken for the complete player bridge | Critical | Q-028 / Phase 5F | **OPEN; ALL CURRENT Q-028 OUTPUT INELIGIBLE** | Former A/B/C observations are diagnostic only. Require a fresh qualifying family predicate before any exact-index experiment or bridge claim. |
| R-033 | Interpreter and normal DRC can place FS transitions on different fixed host frames | High | Q-028 observer acceptance | **OPEN; Q-028 BLOCKED** | Freeze each engine prospectively from two fresh capture-off repeats and audit the schedule hash before any outcome matrix. Never infer or repair one engine's schedule from the other. |
...
| 2026-08-12 | Q-021 mode 2 | **A byte-exact transport observation is narrower than buffer ownership.** The same frame can prove authoritative source, FIFO stream, physical destination, and stack boundary exactly even when later execution reuses the destination for an unknown purpose. | Accept only the predeclared first-post-transfer boundary; record stack direction and R15 in the same capture; treat later mutation as a new ownership risk, not proof of either corruption or intended consumption. |
| 2026-08-12 | Q-023 mailbox correction | **A required dormant byte fix can be proved without invalidating accepted evidence or pretending it executed.** Source-select the corrected handler only in a validation ACTIVE, keep CONTROL equal to the accepted default, and pin the sole opcode/literal delta plus every prior identity. | Carry the corrected Q-023 arm into the next independent shadow gate; do not call static address correction runtime mailbox or COMM validation. |
| 2026-08-12 | Q-026 bounded CMDINT | **Boot initialization does not prove a low-water canary survives until a later racing interrupt.** The failed pilot showed the marker cleared before admission even though the ISR initializer ran. | Refresh and read back the canary at the exact admitted CMD boundary before switching stacks, check it after return, and preserve the failed trace as diagnostic evidence. Keep a direct-invocation precursor narrower than cmd `$3F` shadow, bridge, authority, or cadence claims. |
| 2026-08-13 | Q-027 bounded cmd `$3F` transport | **A real ACTIVE workload can produce a deterministic transient sampled-render difference even when it never writes the framebuffer.** The interpreter observer records one exact `T_I+2` CRC delta while normal DRC remains CRC-equal; both converge at predeclared exact boundaries. | Preserve and report every raw mismatch, bind boundaries to the transaction, require exact repeatability and uninterrupted terminal convergence, corroborate under uninterrupted DRC, and never generalize this narrow gate into render equivalence or a cadence/FPS claim. |
| 2026-08-13 | Q-028 evidence repair | **Interpreter-only completion, reproducible-but-unretained raw data, supplied equality booleans, and parser-only mutation coverage do not satisfy a two-engine gate.** | Freeze engine schedules independently, retain all exact raw members, compare applied inputs byte-for-byte, mutate real accepted-format inputs through production validators, and keep the canonical status blocked until a fresh completed-diff audit. |
