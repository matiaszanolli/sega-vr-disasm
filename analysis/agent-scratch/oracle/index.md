...
| tools/libretro-profiling/VRD_PROFILING.md | Current profiling and normal-1P control gate | `validate_1p_control.py`, `make control-rom`, complete input replay, ≥18K-frame minimum, exact live-reference/hook-bypass comparison, state/hook/FB/COMM liveness | Current VR60 measurement authority; VR60-007 exact-prefix preflight met every functional check; VR60-008 full control failed first on state order at frame 4536; VR60-009 diagnosis is next |
...
22. **Validator finding-list order is not event chronology.** VR60-008's `result.json` lists scene findings before state order, but the archived raw samples place the `$C87E` `$0000->$0008` discontinuity at frame 4536 while `$FF0002` is still `$00884CBC`; sampled COMM0_HI becomes continuously non-zero at frame 4539 and the scene first changes at frame 5128. Use `frames.csv` and `watch.csv` to order observations, not JSON list position. The later hook/framebuffer/Slave-SH2 failures are additional findings with no established causal relationship. VR60-009 must trace all byte/word/long writes overlapping both `$C87E` and `$FF0002` through every emulator memory-write path, logging emulated frame, exact writer PC, access address/width, old value, and new value through the historical 4536/5128 boundaries. The complete hash-verified archive is indexed by `analysis/evidence/vr60-008-full-control/README.md`.
...
| Normal-1P durable control validation | tools/libretro-profiling/VRD_PROFILING.md § Normal-1P control validator | VR60_STATUS.md § Current milestone; VR60-007 proves bounded liveness; VR60-008's full run failed first on state order at frame 4536 and is preserved under `analysis/evidence/vr60-008-full-control/`; VR60-009 must diagnose it before retry; candidate/reference ROMs must match outside the reviewed 8-byte hook delta |
