; ============================================================================
; Heading from Position
; ROM Range: $009040-$009064 (36 bytes)
; ============================================================================
; Computes heading from entity position data. Camera correction applied.
;
; Entry: none
; Uses: D0
; ============================================================================

heading_from_position:
        move.w  ($FFFF903C).w,d0        ; $3038 $903C — base position
        add.w   ($FFFF9096).w,d0        ; $D078 $9096 — add offset
        tst.w    ($FFFFC04C).w          ; $4A78 $C04C — camera flag
        beq.s   .no_cam
        sub.w   ($FFFF9046).w,d0        ; $9078 $9046 — camera correction
.no_cam:
        asr.w   #6,d0                 ; Scale down
        btst    #7,($FFFFFDA8).w        ; $0838 $0007 $FDA8 — mirror flag
        beq.s   .store
        neg.w   d0                    ; Mirror heading
.store:
        move.w  d0,($FFFF8002).w        ; $31C0 $8002 — store heading
        rts
