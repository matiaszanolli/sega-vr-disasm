; ============================================================================
; Z80 Sound Program Upload + FM Key-On — load driver and key-on helper
; ROM Range: $030BF6-$030C8A (148 bytes)
; ============================================================================
; Two parts:
;   $030BF6: Requests Z80 bus, uploads sound program from $031688 to Z80
;     RAM ($A00000, $28D bytes), uploads DAC samples from $031915 to
;     $A01000 ($1000 bytes). Resets Z80 (low→14 NOPs→high), releases bus,
;     branches to silence handler ($030B90).
;   $030C6E (fm_key_on): Checks mute (bit 1) and key-off (bit 2) flags.
;     If neither set, writes key-on register $28 with channel number
;     (A5+$01 OR $F0). Falls through to fm_write_wrapper.
;
; Entry: A5 = FM channel structure pointer (for key-on entry)
; Entry: A6 = sound driver state pointer (loaded at $FF8500 for upload)
; Uses: D0, D1, A0, A1, A5, A6
; Confidence: high
; ============================================================================

z80_sound_program_upload_fm_key_on:
        LEA     $00FF8500,A6                    ; $030BF6
        MOVE.W  #$0100,Z80_BUSREQ                ; $030BFC
.wait_z80_bus:
        BTST    #0,Z80_BUSREQ                    ; $030C04
        BNE.S  .wait_z80_bus                    ; $030C0C
        DC.W    $41FA,$0A78         ; LEA     $031688(PC),A0; $030C0E
        LEA     Z80_RAM,A1                    ; $030C12
        MOVE.W  #$028C,D0                       ; $030C18
.copy_program:
        MOVE.B  (A0)+,(A1)+                     ; $030C1C
        DBRA    D0,.copy_program                ; $030C1E
        DC.W    $41FA,$0CF1         ; LEA     $031915(PC),A0; $030C22
        LEA     $00A01000,A1                    ; $030C26
        MOVE.W  #$0FFF,D0                       ; $030C2C
.copy_dac:
        MOVE.B  (A0)+,(A1)+                     ; $030C30
        DBRA    D0,.copy_dac                    ; $030C32
        MOVE.W  #$0000,Z80_RESET                ; $030C36
        NOP                                     ; $030C3E
        NOP                                     ; $030C40
        NOP                                     ; $030C42
        NOP                                     ; $030C44
        NOP                                     ; $030C46
        NOP                                     ; $030C48
        NOP                                     ; $030C4A
        NOP                                     ; $030C4C
        NOP                                     ; $030C4E
        NOP                                     ; $030C50
        NOP                                     ; $030C52
        NOP                                     ; $030C54
        NOP                                     ; $030C56
        NOP                                     ; $030C58
        MOVE.W  #$0100,Z80_RESET                ; $030C5A
        MOVE.W  #$0000,Z80_BUSREQ                ; $030C62
        DC.W    $6000,$FF24         ; BRA.W  $030B90; $030C6A
        BTST    #1,(A5)                         ; $030C6E
        BNE.S  .skip_key_on                     ; $030C72
        BTST    #2,(A5)                         ; $030C74
        BNE.S  .skip_key_on                     ; $030C78
        MOVEQ   #$28,D0                         ; $030C7A
        MOVE.B  $0001(A5),D1                    ; $030C7C
        ORI.B  #$F0,D1                          ; $030C80
        DC.W    $6000,$0034         ; BRA.W  $030CBA; $030C84
.skip_key_on:
        RTS                                     ; $030C88
