; ============================================================================
; SH2 COMM Transfer Setup A (Horizontal World Data)
; ROM Range: $00EFC2-$00F034 (114 bytes)
; ============================================================================
; Sets up SH2 COMM transfer block at $FF6100 with world coordinate data
; from $FF2000-$FF2004, callback $222BDAE6. Signals SH2 via COMM registers.
;
; Entry: D0 = parameter value
; Uses: A1
; ============================================================================

comm_transfer_setup_a:
        lea     $00FF6100,a1            ; SH2 COMM base
        move.w  #$0001,$0000(a1)        ; Command = 1
        move.w  #$0000,$0002(a1)        ; Param 0 = 0
        move.w  $00FF2002,$0004(a1)     ; Copy X coord
        move.w  $00FF2004,$0006(a1)     ; Copy Y coord
        move.w  $00FF2000,$0008(a1)     ; Copy Z coord
        move.w  d0,$000A(a1)            ; Store D0
        move.w  #$0000,$000C(a1)        ; Clear field C
        move.w  #$0000,$000E(a1)        ; Clear field E
        move.l  #$222BDAE6,$0010(a1)    ; SH2 callback addr
        adda.l  #$00000014,a1           ; Advance past block
        move.w  #$0044,MARS_DREQ_LEN    ; Set DREQ length
        move.b  #$04,MARS_DREQ_CTRL+1   ; Set DMA mode
.wait:  tst.b   COMM0_HI                ; Wait SH2 ready
        bne.s   .wait                   ;   (poll loop)
        move.b  #$2A,COMM0_LO           ; Send command $2A
        move.b  #$01,COMM0_HI           ; Signal SH2
        rts
