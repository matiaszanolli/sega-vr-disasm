...
loc_0047F4:
        ...
        RTS                                     ; $004834
; ----------------------------------------------------------------------------
; QuadMemoryFill ($004836) - Quad memory fill
; ----------------------------------------------------------------------------
QuadMemoryFill:
        JSR     loc_00483A(PC)                  ; $004836
loc_00483A:
; ----------------------------------------------------------------------------
; MemoryFillWaterfall1 ($00483A) - Memory fill waterfall 1
; ----------------------------------------------------------------------------
MemoryFillWaterfall1:
        JSR     loc_00483E(PC)                  ; $00483A
loc_00483E:
; ----------------------------------------------------------------------------
; MemoryFillWaterfall2 ($00483E) - Memory fill waterfall 2
; ----------------------------------------------------------------------------
MemoryFillWaterfall2:
        JSR     loc_004842(PC)                  ; $00483E
loc_004842:
; ----------------------------------------------------------------------------
; MemoryFill60A1 ($004842) - Memory fill 60A1
; ----------------------------------------------------------------------------
MemoryFill60A1:
        JSR     loc_004846(PC)                  ; $004842
loc_004846:
; ----------------------------------------------------------------------------
; MemoryFill60A4 ($004846) - Memory fill 60A4
; ----------------------------------------------------------------------------
MemoryFill60A4:
        MOVE.L  D1,(A1)+                        ; $004846
        MOVE.L  D1,(A1)+                        ; $004848
        ...
        MOVE.L  D1,(A1)+                        ; $004854
; ----------------------------------------------------------------------------
; UnrolledFill96 ($004856) - Unrolled fill 96
; ----------------------------------------------------------------------------
UnrolledFill96:
        MOVE.L  D1,(A1)+                        ; $004856
        MOVE.L  D1,(A1)+                        ; $004858
        ...
        MOVE.L  D1,(A1)+                        ; $00485C
; ----------------------------------------------------------------------------
; UnrolledFill112 ($00485E) - Unrolled fill 112
; ----------------------------------------------------------------------------
UnrolledFill112:
        MOVE.L  D1,(A1)+                        ; $00485E
        MOVE.L  D1,(A1)+                        ; $004860
        ...
        MOVE.L  D1,(A6)                         ; $0048B6
; ----------------------------------------------------------------------------
; UnrolledFill32 ($0048B8) - Unrolled fill 32
; ----------------------------------------------------------------------------
UnrolledFill32:
        MOVE.L  D1,(A6)                         ; $0048B8
        MOVE.L  D1,(A6)                         ; $0048BA
        ...
        RTS                                     ; $0048C8
; ----------------------------------------------------------------------------
; TripleMemoryFill ($0048CA) - Triple memory fill
; ----------------------------------------------------------------------------
TripleMemoryFill:
        JSR     loc_0048CE(PC)                  ; $0048CA
loc_0048CE:
; ----------------------------------------------------------------------------
; MemoryFillWaterfall3 ($0048CE) - Memory fill waterfall 3
; ----------------------------------------------------------------------------
MemoryFillWaterfall3:
        JSR     loc_0048D2(PC)                  ; $0048CE
loc_0048D2:
; ----------------------------------------------------------------------------
; MemoryFillWaterfall4 ($0048D2) - Memory fill waterfall 4
; ----------------------------------------------------------------------------
...
loc_0048FA:
        ...
        MOVE.L  (A1)+,(A2)+                     ; $0048FC
; ----------------------------------------------------------------------------
; UnrolledFill60 ($0048FE) - Unrolled fill 60
; ----------------------------------------------------------------------------
UnrolledFill60:
        MOVE.L  (A1)+,(A2)+                     ; $0048FE
        MOVE.L  (A1)+,(A2)+                     ; $004900
        ...
        MOVE.L  (A1)+,(A2)+                     ; $00491E
; ----------------------------------------------------------------------------
; FastCopy20 ($004920) - Fast copy 20 bytes
; ----------------------------------------------------------------------------
FastCopy20:
        MOVE.L  (A1)+,(A2)+                     ; $004920
; ----------------------------------------------------------------------------
; FastCopy16 ($004922) - Fast copy 16 bytes
; ----------------------------------------------------------------------------
FastCopy16:
        MOVE.L  (A1)+,(A2)+                     ; $004922
        MOVE.L  (A1)+,(A2)+                     ; $004924
        ...
        RTS                                     ; $00496C
; ----------------------------------------------------------------------------
; random_number_gen ($00496E) - PRNG using $EF00.W (6 calls)
; ----------------------------------------------------------------------------
random_number_gen:
        MOVE.L  D1,-(A7)                        ; $00496E
...
loc_004998:
; ----------------------------------------------------------------------------
; WaitForVBlank ($004998) - Wait for V-blank
; ----------------------------------------------------------------------------
WaitForVBlank:
...
loc_0049AA:
; ----------------------------------------------------------------------------
; SetDisplayParams ($0049AA) - Set display parameters
; ----------------------------------------------------------------------------
SetDisplayParams:
...
loc_0049EE:
; ----------------------------------------------------------------------------
; reset_scroll_vars ($0049EE) - Clear scroll variables (3 calls)
; ----------------------------------------------------------------------------
reset_scroll_vars:
...
loc_0058C8:
; ----------------------------------------------------------------------------
; sprite_input_check ($0058C8) - Sprite/input validation (6 calls)
; ----------------------------------------------------------------------------
sprite_input_check:
...
loc_005908:
; ----------------------------------------------------------------------------
; sprite_update_check ($005908) - Sprite update conditional (6 calls)
; ----------------------------------------------------------------------------
sprite_update_check:
...
loc_00593C:
; ----------------------------------------------------------------------------
; sprite_state_process ($00593C) - Sprite state processing (6 calls)
; ----------------------------------------------------------------------------
sprite_state_process:
...
loc_0059EC:
; ----------------------------------------------------------------------------
; table_lookup ($0059EC) - Table lookup
; ----------------------------------------------------------------------------
table_lookup:
        MOVE.W  D7,-(A7)                        ; $0059EC
