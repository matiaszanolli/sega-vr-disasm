; ============================================================================
; Code Section ($010200-$0121FF)
; ============================================================================

        org     $010200

code_10200:
        dc.w    $1050        ; $010200
        dc.w    $303C        ; $010202
        dc.w    $00A8        ; $010204
        dc.w    $323C        ; $010206
        dc.w    $0010        ; $010208
        dc.w    $4EBA        ; $01020A
        dc.w    $E14E        ; $01020C
        dc.w    $207C        ; $01020E
        dc.w    $0602        ; $010210
        dc.w    $0000        ; $010212
        dc.w    $3038        ; $010214
        dc.w    $A022        ; $010216
        dc.w    $EF48        ; $010218
        dc.w    $3200        ; $01021A
        dc.w    $E548        ; $01021C
        dc.w    $D041        ; $01021E
        dc.w    $41F0        ; $010220
        dc.w    $0000        ; $010222
        dc.w    $227C        ; $010224
        dc.w    $2403        ; $010226
        dc.w    $10CC        ; $010228
        dc.w    $303C        ; $01022A
        dc.w    $0028        ; $01022C
        dc.w    $323C        ; $01022E
        dc.w    $0010        ; $010230
        dc.w    $4EBA        ; $010232
        dc.w    $E126        ; $010234
        dc.w    $5878        ; $010236
        dc.w    $C87E        ; $010238
        dc.w    $33FC        ; $01023A
        dc.w    $0020        ; $01023C
        dc.w    $00FF        ; $01023E
        dc.w    $0008        ; $010240
        dc.w    $4E75        ; $010242
        dc.w    $4240        ; $010244
        dc.w    $4EBA        ; $010246
        dc.w    $E2E4        ; $010248
        dc.w    $2078        ; $01024A
        dc.w    $A018        ; $01024C
        dc.w    $6100        ; $01024E
        dc.w    $0546        ; $010250
        dc.w    $4EB9        ; $010252
        dc.w    $0088        ; $010254
        dc.w    $179E        ; $010256
        dc.w    $0C78        ; $010258
        dc.w    $0001        ; $01025A
        dc.w    $A036        ; $01025C
        dc.w    $6700        ; $01025E
        dc.w    $00DC        ; $010260
        dc.w    $3238        ; $010262
        dc.w    $C86C        ; $010264
        dc.w    $0801        ; $010266
        dc.w    $0004        ; $010268
        dc.w    $6600        ; $01026A
        dc.w    $0074        ; $01026C
        dc.w    $3401        ; $01026E
        dc.w    $0202        ; $010270
        dc.w    $00E0        ; $010272
        dc.w    $6700        ; $010274
        dc.w    $00C2        ; $010276
        dc.w    $11FC        ; $010278
        dc.w    $0001        ; $01027A
        dc.w    $A02C        ; $01027C
        dc.w    $11FC        ; $01027E
        dc.w    $0000        ; $010280
        dc.w    $A02D        ; $010282
        dc.w    $11FC        ; $010284
        dc.w    $00A8        ; $010286
        dc.w    $C8A4        ; $010288
        dc.w    $0801        ; $01028A
        dc.w    $0007        ; $01028C
        dc.w    $6600        ; $01028E
        dc.w    $0048        ; $010290
        dc.w    $3038        ; $010292
        dc.w    $A024        ; $010294
        dc.w    $0C00        ; $010296
        dc.w    $0003        ; $010298
        dc.w    $6700        ; $01029A
        dc.w    $003C        ; $01029C
        dc.w    $0C00        ; $01029E
        dc.w    $0008        ; $0102A0
        dc.w    $6700        ; $0102A2
        dc.w    $003C        ; $0102A4
        dc.w    $4241        ; $0102A6
        dc.w    $1238        ; $0102A8
        dc.w    $A020        ; $0102AA
        dc.w    $2078        ; $0102AC
        dc.w    $A018        ; $0102AE
        dc.w    $1180        ; $0102B0
        dc.w    $1000        ; $0102B2
        dc.w    $0838        ; $0102B4
        dc.w    $0001        ; $0102B6
        dc.w    $A014        ; $0102B8
        dc.w    $6700        ; $0102BA
        dc.w    $000A        ; $0102BC
        dc.w    $2078        ; $0102BE
        dc.w    $A01C        ; $0102C0
        dc.w    $1180        ; $0102C2
        dc.w    $1000        ; $0102C4
        dc.w    $5238        ; $0102C6
        dc.w    $A020        ; $0102C8
        dc.w    $0C38        ; $0102CA
        dc.w    $0003        ; $0102CC
        dc.w    $A020        ; $0102CE
        dc.w    $6C00        ; $0102D0
        dc.w    $0006        ; $0102D2
        dc.w    $6000        ; $0102D4
        dc.w    $0078        ; $0102D6
        dc.w    $5878        ; $0102D8
        dc.w    $C87E        ; $0102DA
        dc.w    $6000        ; $0102DC
        dc.w    $0074        ; $0102DE
        dc.w    $4A38        ; $0102E0
        dc.w    $A020        ; $0102E2
        dc.w    $6700        ; $0102E4
        dc.w    $0068        ; $0102E6
        dc.w    $4241        ; $0102E8
        dc.w    $1238        ; $0102EA
        dc.w    $A020        ; $0102EC
        dc.w    $2078        ; $0102EE
        dc.w    $A018        ; $0102F0
        dc.w    $11BC        ; $0102F2
        dc.w    $0020        ; $0102F4
        dc.w    $1000        ; $0102F6
        dc.w    $0838        ; $0102F8
        dc.w    $0001        ; $0102FA
        dc.w    $A014        ; $0102FC
        dc.w    $6700        ; $0102FE
        dc.w    $000C        ; $010300
        dc.w    $2078        ; $010302
        dc.w    $A01C        ; $010304
        dc.w    $11BC        ; $010306
        dc.w    $0020        ; $010308
        dc.w    $1000        ; $01030A
        dc.w    $5338        ; $01030C
        dc.w    $A020        ; $01030E
        dc.w    $4241        ; $010310
        dc.w    $1238        ; $010312
        dc.w    $A020        ; $010314
        dc.w    $2078        ; $010316
        dc.w    $A018        ; $010318
        dc.w    $11BC        ; $01031A
        dc.w    $0020        ; $01031C
        dc.w    $1000        ; $01031E
        dc.w    $0838        ; $010320
        dc.w    $0001        ; $010322
        dc.w    $A014        ; $010324
        dc.w    $6700        ; $010326
        dc.w    $000C        ; $010328
        dc.w    $2078        ; $01032A
        dc.w    $A01C        ; $01032C
        dc.w    $11BC        ; $01032E
        dc.w    $0020        ; $010330
        dc.w    $1000        ; $010332
        dc.w    $6000        ; $010334
        dc.w    $0018        ; $010336
        dc.w    $6100        ; $010338
        dc.w    $0512        ; $01033A
        dc.w    $4EB9        ; $01033C
        dc.w    $0088        ; $01033E
        dc.w    $FB36        ; $010340
        dc.w    $0838        ; $010342
        dc.w    $0006        ; $010344
        dc.w    $C80E        ; $010346
        dc.w    $6604        ; $010348
        dc.w    $4278        ; $01034A
        dc.w    $A036        ; $01034C
        dc.w    $5978        ; $01034E
        dc.w    $C87E        ; $010350
        dc.w    $33FC        ; $010352
        dc.w    $0018        ; $010354
        dc.w    $00FF        ; $010356
        dc.w    $0008        ; $010358
        dc.w    $4E75        ; $01035A
        dc.w    $4240        ; $01035C
        dc.w    $6100        ; $01035E
        dc.w    $E1CC        ; $010360
        dc.w    $4EBA        ; $010362
        dc.w    $B320        ; $010364
        dc.w    $6100        ; $010366
        dc.w    $03B4        ; $010368
        dc.w    $207C        ; $01036A
        dc.w    $0601        ; $01036C
        dc.w    $C300        ; $01036E
        dc.w    $227C        ; $010370
        dc.w    $2400        ; $010372
        dc.w    $E030        ; $010374
        dc.w    $303C        ; $010376
        dc.w    $0080        ; $010378
        dc.w    $323C        ; $01037A
        dc.w    $0020        ; $01037C
        dc.w    $4EBA        ; $01037E
        dc.w    $DFDA        ; $010380
        dc.w    $43F9        ; $010382
        dc.w    $2402        ; $010384
        dc.w    $F0C0        ; $010386
        dc.w    $45F8        ; $010388
        dc.w    $FA48        ; $01038A
        dc.w    $7000        ; $01038C
        dc.w    $1038        ; $01038E
        dc.w    $FEB1        ; $010390
        dc.w    $D040        ; $010392
        dc.w    $D040        ; $010394
        dc.w    $D040        ; $010396
        dc.w    $3200        ; $010398
        dc.w    $D040        ; $01039A
        dc.w    $D041        ; $01039C
        dc.w    $D040        ; $01039E
        dc.w    $D5C0        ; $0103A0
        dc.w    $7000        ; $0103A2
        dc.w    $1038        ; $0103A4
        dc.w    $FEA5        ; $0103A6
        dc.w    $D040        ; $0103A8
        dc.w    $D040        ; $0103AA
        dc.w    $D040        ; $0103AC
        dc.w    $5840        ; $0103AE
        dc.w    $D5C0        ; $0103B0
        dc.w    $6100        ; $0103B2
        dc.w    $0252        ; $0103B4
        dc.w    $5878        ; $0103B6
        dc.w    $C87E        ; $0103B8
        dc.w    $33FC        ; $0103BA
        dc.w    $0020        ; $0103BC
        dc.w    $00FF        ; $0103BE
        dc.w    $0008        ; $0103C0
        dc.w    $4E75        ; $0103C2
        dc.w    $4240        ; $0103C4
        dc.w    $4EBA        ; $0103C6
        dc.w    $E164        ; $0103C8
        dc.w    $2078        ; $0103CA
        dc.w    $A01C        ; $0103CC
        dc.w    $6100        ; $0103CE
        dc.w    $03C6        ; $0103D0
        dc.w    $4EB9        ; $0103D2
        dc.w    $0088        ; $0103D4
        dc.w    $179E        ; $0103D6
        dc.w    $0C78        ; $0103D8
        dc.w    $0001        ; $0103DA
        dc.w    $A036        ; $0103DC
        dc.w    $6700        ; $0103DE
        dc.w    $00A2        ; $0103E0
        dc.w    $3238        ; $0103E2
        dc.w    $C86C        ; $0103E4
        dc.w    $0801        ; $0103E6
        dc.w    $0004        ; $0103E8
        dc.w    $6600        ; $0103EA
        dc.w    $0062        ; $0103EC
        dc.w    $3401        ; $0103EE
        dc.w    $0202        ; $0103F0
        dc.w    $00E0        ; $0103F2
        dc.w    $6700        ; $0103F4
        dc.w    $0088        ; $0103F6
        dc.w    $11FC        ; $0103F8
        dc.w    $0001        ; $0103FA
        dc.w    $A02C        ; $0103FC
        dc.w    $11FC        ; $0103FE
        dc.w    $0000        ; $010400
        dc.w    $A02D        ; $010402
        dc.w    $11FC        ; $010404
        dc.w    $00A8        ; $010406
        dc.w    $C8A4        ; $010408
        dc.w    $0801        ; $01040A
        dc.w    $0007        ; $01040C
        dc.w    $6600        ; $01040E
        dc.w    $0036        ; $010410
        dc.w    $3038        ; $010412
        dc.w    $A024        ; $010414
        dc.w    $0C00        ; $010416
        dc.w    $0003        ; $010418
        dc.w    $6700        ; $01041A
        dc.w    $002A        ; $01041C
        dc.w    $0C00        ; $01041E
        dc.w    $0008        ; $010420
        dc.w    $6700        ; $010422
        dc.w    $002A        ; $010424
        dc.w    $4241        ; $010426
        dc.w    $1238        ; $010428
        dc.w    $A020        ; $01042A
        dc.w    $2078        ; $01042C
        dc.w    $A01C        ; $01042E
        dc.w    $1180        ; $010430
        dc.w    $1000        ; $010432
        dc.w    $5238        ; $010434
        dc.w    $A020        ; $010436
        dc.w    $0C38        ; $010438
        dc.w    $0003        ; $01043A
        dc.w    $A020        ; $01043C
        dc.w    $6C00        ; $01043E
        dc.w    $0006        ; $010440
        dc.w    $6000        ; $010442
        dc.w    $0050        ; $010444
        dc.w    $5878        ; $010446
        dc.w    $C87E        ; $010448
        dc.w    $6000        ; $01044A
        dc.w    $004C        ; $01044C
        dc.w    $4241        ; $01044E
        dc.w    $1238        ; $010450
        dc.w    $A020        ; $010452
        dc.w    $2078        ; $010454
        dc.w    $A01C        ; $010456
        dc.w    $11BC        ; $010458
        dc.w    $0020        ; $01045A
        dc.w    $1000        ; $01045C
        dc.w    $4A38        ; $01045E
        dc.w    $A020        ; $010460
        dc.w    $6700        ; $010462
        dc.w    $0030        ; $010464
        dc.w    $5338        ; $010466
        dc.w    $A020        ; $010468
        dc.w    $4241        ; $01046A
        dc.w    $1238        ; $01046C
        dc.w    $A020        ; $01046E
        dc.w    $2078        ; $010470
        dc.w    $A01C        ; $010472
        dc.w    $11BC        ; $010474
        dc.w    $0020        ; $010476
        dc.w    $1000        ; $010478
        dc.w    $6000        ; $01047A
        dc.w    $0018        ; $01047C
        dc.w    $6100        ; $01047E
        dc.w    $03CC        ; $010480
        dc.w    $4EB9        ; $010482
        dc.w    $0088        ; $010484
        dc.w    $FB36        ; $010486
        dc.w    $0838        ; $010488
        dc.w    $0006        ; $01048A
        dc.w    $C80E        ; $01048C
        dc.w    $6604        ; $01048E
        dc.w    $4278        ; $010490
        dc.w    $A036        ; $010492
        dc.w    $5978        ; $010494
        dc.w    $C87E        ; $010496
        dc.w    $33FC        ; $010498
        dc.w    $0018        ; $01049A
        dc.w    $00FF        ; $01049C
        dc.w    $0008        ; $01049E
        dc.w    $4E75        ; $0104A0
        dc.w    $4240        ; $0104A2
        dc.w    $4EBA        ; $0104A4
        dc.w    $E086        ; $0104A6
        dc.w    $4EBA        ; $0104A8
        dc.w    $B230        ; $0104AA
        dc.w    $207C        ; $0104AC
        dc.w    $0601        ; $0104AE
        dc.w    $4000        ; $0104B0
        dc.w    $227C        ; $0104B2
        dc.w    $2401        ; $0104B4
        dc.w    $4034        ; $0104B6
        dc.w    $303C        ; $0104B8
        dc.w    $00D8        ; $0104BA
        dc.w    $323C        ; $0104BC
        dc.w    $0050        ; $0104BE
        dc.w    $4EBA        ; $0104C0
        dc.w    $DE98        ; $0104C2
        dc.w    $4A78        ; $0104C4
        dc.w    $A02E        ; $0104C6
        dc.w    $6A3C        ; $0104C8
        dc.w    $11FC        ; $0104CA
        dc.w    $0001        ; $0104CC
        dc.w    $A02C        ; $0104CE
        dc.w    $11FC        ; $0104D0
        dc.w    $0001        ; $0104D2
        dc.w    $A02D        ; $0104D4
        dc.w    $5378        ; $0104D6
        dc.w    $A030        ; $0104D8
        dc.w    $642A        ; $0104DA
        dc.w    $31FC        ; $0104DC
        dc.w    $0002        ; $0104DE
        dc.w    $A036        ; $0104E0
        dc.w    $11FC        ; $0104E2
        dc.w    $0001        ; $0104E4
        dc.w    $C809        ; $0104E6
        dc.w    $11FC        ; $0104E8
        dc.w    $0001        ; $0104EA
        dc.w    $C80A        ; $0104EC
        dc.w    $08F8        ; $0104EE
        dc.w    $0007        ; $0104F0
        dc.w    $C80E        ; $0104F2
        dc.w    $11FC        ; $0104F4
        dc.w    $0001        ; $0104F6
        dc.w    $C802        ; $0104F8
        dc.w    $4EB9        ; $0104FA
        dc.w    $0088        ; $0104FC
        dc.w    $205E        ; $0104FE
        dc.w    $31FC        ; $010500
        dc.w    $0BB8        ; $010502
        dc.w    $A030        ; $010504
        dc.w    $2078        ; $010506
        dc.w    $A018        ; $010508
        dc.w    $0838        ; $01050A
        dc.w    $0000        ; $01050C
        dc.w    $A014        ; $01050E
        dc.w    $6600        ; $010510
        dc.w    $0006        ; $010512
        dc.w    $2078        ; $010514
        dc.w    $A01C        ; $010516
        dc.w    $4A38        ; $010518
        dc.w    $A02C        ; $01051A
        dc.w    $6700        ; $01051C
        dc.w    $0022        ; $01051E
        dc.w    $2848        ; $010520
        dc.w    $4240        ; $010522
        dc.w    $1010        ; $010524
        dc.w    $0C00        ; $010526
        dc.w    $0020        ; $010528
        dc.w    $6700        ; $01052A
        dc.w    $0014        ; $01052C
        dc.w    $0C00        ; $01052E
        dc.w    $0003        ; $010530
        dc.w    $6700        ; $010532
        dc.w    $000C        ; $010534
        dc.w    $227C        ; $010536
        dc.w    $2403        ; $010538
        dc.w    $4060        ; $01053A
        dc.w    $6100        ; $01053C
        dc.w    $0136        ; $01053E
        dc.w    $4A38        ; $010540
        dc.w    $A02C        ; $010542
        dc.w    $6700        ; $010544
        dc.w    $0022        ; $010546
        dc.w    $3014        ; $010548
        dc.w    $0240        ; $01054A
        dc.w    $00FF        ; $01054C
        dc.w    $0C00        ; $01054E
        dc.w    $0020        ; $010550
        dc.w    $6700        ; $010552
        dc.w    $0014        ; $010554
        dc.w    $0C00        ; $010556
        dc.w    $0003        ; $010558
        dc.w    $6700        ; $01055A
        dc.w    $000C        ; $01055C
        dc.w    $227C        ; $01055E
        dc.w    $2403        ; $010560
        dc.w    $4090        ; $010562
        dc.w    $6100        ; $010564
        dc.w    $010E        ; $010566
        dc.w    $4A38        ; $010568
        dc.w    $A02C        ; $01056A
        dc.w    $6700        ; $01056C
        dc.w    $0024        ; $01056E
        dc.w    $102C        ; $010570
        dc.w    $0002        ; $010572
        dc.w    $0240        ; $010574
        dc.w    $00FF        ; $010576
        dc.w    $0C00        ; $010578
        dc.w    $0020        ; $01057A
        dc.w    $6700        ; $01057C
        dc.w    $0014        ; $01057E
        dc.w    $0C00        ; $010580
        dc.w    $0003        ; $010582
        dc.w    $6700        ; $010584
        dc.w    $000C        ; $010586
        dc.w    $227C        ; $010588
        dc.w    $2403        ; $01058A
        dc.w    $40C0        ; $01058C
        dc.w    $6100        ; $01058E
        dc.w    $00E4        ; $010590
        dc.w    $0C78        ; $010592
        dc.w    $0002        ; $010594
        dc.w    $A036        ; $010596
        dc.w    $6700        ; $010598
        dc.w    $0024        ; $01059A
        dc.w    $5338        ; $01059C
        dc.w    $A02D        ; $01059E
        dc.w    $6418        ; $0105A0
        dc.w    $0878        ; $0105A2
        dc.w    $0000        ; $0105A4
        dc.w    $A02C        ; $0105A6
        dc.w    $11FC        ; $0105A8
        dc.w    $0001        ; $0105AA
        dc.w    $A02D        ; $0105AC
        dc.w    $0838        ; $0105AE
        dc.w    $0000        ; $0105B0
        dc.w    $A02C        ; $0105B2
        dc.w    $6704        ; $0105B4
        dc.w    $5378        ; $0105B6
        dc.w    $A02E        ; $0105B8
        dc.w    $6000        ; $0105BA
        dc.w    $0018        ; $0105BC
        dc.w    $4EB9        ; $0105BE
        dc.w    $0088        ; $0105C0
        dc.w    $FB36        ; $0105C2
        dc.w    $0838        ; $0105C4
        dc.w    $0007        ; $0105C6
        dc.w    $C80E        ; $0105C8
        dc.w    $6608        ; $0105CA
        dc.w    $4278        ; $0105CC
        dc.w    $A036        ; $0105CE
        dc.w    $5878        ; $0105D0
        dc.w    $C87E        ; $0105D2
        dc.w    $33FC        ; $0105D4
        dc.w    $0018        ; $0105D6
        dc.w    $00FF        ; $0105D8
        dc.w    $0008        ; $0105DA
        dc.w    $4E75        ; $0105DC
        dc.w    $4A39        ; $0105DE
        dc.w    $00A1        ; $0105E0
        dc.w    $5120        ; $0105E2
        dc.w    $66F8        ; $0105E4
        dc.w    $4239        ; $0105E6
        dc.w    $00A1        ; $0105E8
        dc.w    $5123        ; $0105EA
        dc.w    $31FC        ; $0105EC
        dc.w    $0000        ; $0105EE
        dc.w    $C87E        ; $0105F0
        dc.w    $33FC        ; $0105F2
        dc.w    $0020        ; $0105F4
        dc.w    $00FF        ; $0105F6
        dc.w    $0008        ; $0105F8
        dc.w    $23FC        ; $0105FA
        dc.w    $0089        ; $0105FC
        dc.w    $09AE        ; $0105FE
        dc.w    $00FF        ; $010600
        dc.w    $0002        ; $010602
        dc.w    $4E75        ; $010604
        dc.w    $161A        ; $010606
        dc.w    $6100        ; $010608
        dc.w    $0030        ; $01060A
        dc.w    $323C        ; $01060C
        dc.w    $000A        ; $01060E
        dc.w    $6100        ; $010610
        dc.w    $0044        ; $010612
        dc.w    $5089        ; $010614
        dc.w    $161A        ; $010616
        dc.w    $6100        ; $010618
        dc.w    $0020        ; $01061A
        dc.w    $323C        ; $01061C
        dc.w    $000B        ; $01061E
        dc.w    $6100        ; $010620
        dc.w    $0034        ; $010622
        dc.w    $5089        ; $010624
        dc.w    $121A        ; $010626
        dc.w    $0241        ; $010628
        dc.w    $000F        ; $01062A
        dc.w    $6100        ; $01062C
        dc.w    $0028        ; $01062E
        dc.w    $5089        ; $010630
        dc.w    $161A        ; $010632
        dc.w    $6100        ; $010634
        dc.w    $0004        ; $010636
        dc.w    $4E75        ; $010638
        dc.w    $1203        ; $01063A
        dc.w    $E809        ; $01063C
        dc.w    $0241        ; $01063E
        dc.w    $000F        ; $010640
        dc.w    $6100        ; $010642
        dc.w    $0012        ; $010644
        dc.w    $5089        ; $010646
        dc.w    $3203        ; $010648
        dc.w    $0241        ; $01064A
        dc.w    $000F        ; $01064C
        dc.w    $6100        ; $01064E
        dc.w    $0006        ; $010650
        dc.w    $5089        ; $010652
        dc.w    $4E75        ; $010654
        dc.w    $ED49        ; $010656
        dc.w    $3001        ; $010658
        dc.w    $E349        ; $01065A
        dc.w    $D240        ; $01065C
        dc.w    $207C        ; $01065E
        dc.w    $0602        ; $010660
        dc.w    $3200        ; $010662
        dc.w    $D0C1        ; $010664
        dc.w    $303C        ; $010666
        dc.w    $000C        ; $010668
        dc.w    $323C        ; $01066A
        dc.w    $0010        ; $01066C
        dc.w    $4EBA        ; $01066E
        dc.w    $DCEA        ; $010670
        dc.w    $4E75        ; $010672

; --- Main Code 2 high-freq (9 calls) ---
MC2HighFreq1:
        CMPI.W #$0020,D0        ; $010674
        BEQ $008906BC        ; $010678
        CMPI.W #$0008,D0        ; $01067C
        BEQ $008906C4        ; $010680
        CMPI.W #$0003,D0        ; $010684
        BEQ $008906CC        ; $010688
        CMPI.W #$002E,D0        ; $01068C
        BEQ $008906D4        ; $010690
        CMPI.W #$0021,D0        ; $010694
        BEQ $008906DC        ; $010698
        CMPI.W #$003F,D0        ; $01069C
        BEQ $008906B4        ; $0106A0
        CMPI.W #$005A,D0        ; $0106A4
        BLE $008906E4        ; $0106A8
        CMPI.W #$007A,D0        ; $0106AC
        BLE $008906EC        ; $0106B0
        MOVE.W #$0036,D0        ; $0106B4
        BRA $008906F0        ; $0106B8
        MOVE.W #$0037,D0        ; $0106BC
        BRA $008906F0        ; $0106C0
        MOVE.W #$0038,D0        ; $0106C4
        BRA $008906F0        ; $0106C8
        MOVE.W #$0039,D0        ; $0106CC
        BRA $008906F0        ; $0106D0
        MOVE.W #$0034,D0        ; $0106D4
        BRA $008906F0        ; $0106D8
        MOVE.W #$0035,D0        ; $0106DC
        BRA $008906F0        ; $0106E0
        BCHG #65,D0        ; $0106E4
        BRA $008906F0        ; $0106E8
        BCHG #71,D0        ; $0106EC
        BCLR #0,D0        ; $0106F0
        DC.W $FFFF ; Unknown        ; $0106F4
        LSL.L #6,D0        ; $0106F6
        MOVE.L D0,D1        ; $0106F8
        LSL.L #1,D0        ; $0106FA
        ADD.L D0,D1        ; $0106FC
        LSL.L #1,D0        ; $0106FE
        ADD.L D0,D1        ; $010700
        LSL.L #1,D0        ; $010702
        ADD.L D1,D0        ; $010704
        MOVEA.L #$06024000,A0        ; $010706
        ADDA.L D0,A0        ; $01070C
        MOVE.W #$0018,D0        ; $01070E
        MOVE.W #$0028,D1        ; $010712
        JSR $0088E35A(PC)        ; $010716
        RTS        ; $01071A
        dc.w    $207C        ; $01071C
        dc.w    $0601        ; $01071E
        dc.w    $4000        ; $010720
        dc.w    $227C        ; $010722
        dc.w    $2401        ; $010724
        dc.w    $4034        ; $010726
        dc.w    $303C        ; $010728
        dc.w    $00D8        ; $01072A
        dc.w    $323C        ; $01072C
        dc.w    $0050        ; $01072E
        dc.w    $4EBA        ; $010730
        dc.w    $DC28        ; $010732
        dc.w    $207C        ; $010734
        dc.w    $0601        ; $010736
        dc.w    $9700        ; $010738
        dc.w    $227C        ; $01073A
        dc.w    $2400        ; $01073C
        dc.w    $80A0        ; $01073E
        dc.w    $303C        ; $010740
        dc.w    $0058        ; $010742
        dc.w    $323C        ; $010744
        dc.w    $0010        ; $010746
        dc.w    $4EBA        ; $010748
        dc.w    $DC10        ; $01074A
        dc.w    $207C        ; $01074C
        dc.w    $0601        ; $01074E
        dc.w    $9C80        ; $010750
        dc.w    $227C        ; $010752
        dc.w    $2400        ; $010754
        dc.w    $A0A0        ; $010756
        dc.w    $303C        ; $010758
        dc.w    $0058        ; $01075A
        dc.w    $323C        ; $01075C
        dc.w    $0010        ; $01075E
        dc.w    $4EBA        ; $010760
        dc.w    $DBF8        ; $010762
        dc.w    $207C        ; $010764
        dc.w    $0601        ; $010766
        dc.w    $9000        ; $010768
        dc.w    $227C        ; $01076A
        dc.w    $2400        ; $01076C
        dc.w    $8060        ; $01076E
        dc.w    $303C        ; $010770
        dc.w    $0038        ; $010772
        dc.w    $323C        ; $010774
        dc.w    $0020        ; $010776
        dc.w    $4EBA        ; $010778
        dc.w    $DBE0        ; $01077A
        dc.w    $207C        ; $01077C
        dc.w    $0601        ; $01077E
        dc.w    $A200        ; $010780
        dc.w    $227C        ; $010782
        dc.w    $2400        ; $010784
        dc.w    $4C60        ; $010786
        dc.w    $303C        ; $010788
        dc.w    $0080        ; $01078A
        dc.w    $323C        ; $01078C
        dc.w    $0010        ; $01078E
        dc.w    $4EBA        ; $010790
        dc.w    $DBC8        ; $010792
        dc.w    $4E75        ; $010794
        dc.w    $5338        ; $010796
        dc.w    $A02D        ; $010798
        dc.w    $640C        ; $01079A
        dc.w    $0878        ; $01079C
        dc.w    $0000        ; $01079E
        dc.w    $A02C        ; $0107A0
        dc.w    $11FC        ; $0107A2
        dc.w    $0000        ; $0107A4
        dc.w    $A02D        ; $0107A6
        dc.w    $4A38        ; $0107A8
        dc.w    $A020        ; $0107AA
        dc.w    $6700        ; $0107AC
        dc.w    $0056        ; $0107AE
        dc.w    $0C38        ; $0107B0
        dc.w    $0001        ; $0107B2
        dc.w    $A020        ; $0107B4
        dc.w    $6700        ; $0107B6
        dc.w    $0066        ; $0107B8
        dc.w    $2848        ; $0107BA
        dc.w    $4240        ; $0107BC
        dc.w    $1010        ; $0107BE
        dc.w    $0C00        ; $0107C0
        dc.w    $0020        ; $0107C2
        dc.w    $6700        ; $0107C4
        dc.w    $000C        ; $0107C6
        dc.w    $227C        ; $0107C8
        dc.w    $2403        ; $0107CA
        dc.w    $4060        ; $0107CC
        dc.w    $6100        ; $0107CE
        dc.w    $FEA4        ; $0107D0
        dc.w    $3014        ; $0107D2
        dc.w    $0240        ; $0107D4
        dc.w    $00FF        ; $0107D6
        dc.w    $0C00        ; $0107D8
        dc.w    $0020        ; $0107DA
        dc.w    $6700        ; $0107DC
        dc.w    $000C        ; $0107DE
        dc.w    $227C        ; $0107E0
        dc.w    $2403        ; $0107E2
        dc.w    $4090        ; $0107E4
        dc.w    $6100        ; $0107E6
        dc.w    $FE8C        ; $0107E8
        dc.w    $4A38        ; $0107EA
        dc.w    $A02C        ; $0107EC
        dc.w    $6700        ; $0107EE
        dc.w    $005A        ; $0107F0
        dc.w    $3038        ; $0107F2
        dc.w    $A024        ; $0107F4
        dc.w    $227C        ; $0107F6
        dc.w    $2403        ; $0107F8
        dc.w    $40C0        ; $0107FA
        dc.w    $6100        ; $0107FC
        dc.w    $FE76        ; $0107FE
        dc.w    $6000        ; $010800
        dc.w    $0048        ; $010802
        dc.w    $4A38        ; $010804
        dc.w    $A02C        ; $010806
        dc.w    $6700        ; $010808
        dc.w    $0040        ; $01080A
        dc.w    $3038        ; $01080C
        dc.w    $A024        ; $01080E
        dc.w    $227C        ; $010810
        dc.w    $2403        ; $010812
        dc.w    $4060        ; $010814
        dc.w    $6100        ; $010816
        dc.w    $FE5C        ; $010818
        dc.w    $6000        ; $01081A
        dc.w    $002E        ; $01081C
        dc.w    $4240        ; $01081E
        dc.w    $1010        ; $010820
        dc.w    $0C00        ; $010822
        dc.w    $0020        ; $010824
        dc.w    $6700        ; $010826
        dc.w    $000C        ; $010828
        dc.w    $227C        ; $01082A
        dc.w    $2403        ; $01082C
        dc.w    $4060        ; $01082E
        dc.w    $6100        ; $010830
        dc.w    $FE42        ; $010832
        dc.w    $4A38        ; $010834
        dc.w    $A02C        ; $010836
        dc.w    $6700        ; $010838
        dc.w    $0010        ; $01083A
        dc.w    $3038        ; $01083C
        dc.w    $A024        ; $01083E
        dc.w    $227C        ; $010840
        dc.w    $2403        ; $010842
        dc.w    $4090        ; $010844
        dc.w    $6100        ; $010846
        dc.w    $FE2C        ; $010848
        dc.w    $4E75        ; $01084A
        dc.w    $3401        ; $01084C
        dc.w    $E04A        ; $01084E
        dc.w    $B438        ; $010850
        dc.w    $A02A        ; $010852
        dc.w    $6600        ; $010854
        dc.w    $001C        ; $010856
        dc.w    $5238        ; $010858
        dc.w    $A02B        ; $01085A
        dc.w    $0C38        ; $01085C
        dc.w    $000F        ; $01085E
        dc.w    $A02B        ; $010860
        dc.w    $6D00        ; $010862
        dc.w    $0012        ; $010864
        dc.w    $11FC        ; $010866
        dc.w    $000C        ; $010868
        dc.w    $A02B        ; $01086A
        dc.w    $3202        ; $01086C
        dc.w    $6000        ; $01086E
        dc.w    $0006        ; $010870
        dc.w    $4238        ; $010872
        dc.w    $A02B        ; $010874
        dc.w    $11C2        ; $010876
        dc.w    $A02A        ; $010878
        dc.w    $0801        ; $01087A
        dc.w    $0002        ; $01087C
        dc.w    $673A        ; $01087E
        dc.w    $11FC        ; $010880
        dc.w    $0001        ; $010882
        dc.w    $A02C        ; $010884
        dc.w    $11FC        ; $010886
        dc.w    $0000        ; $010888
        dc.w    $A02D        ; $01088A
        dc.w    $11FC        ; $01088C
        dc.w    $00A9        ; $01088E
        dc.w    $C8A4        ; $010890
        dc.w    $4A78        ; $010892
        dc.w    $A026        ; $010894
        dc.w    $6718        ; $010896
        dc.w    $5378        ; $010898
        dc.w    $A026        ; $01089A
        dc.w    $0C78        ; $01089C
        dc.w    $0033        ; $01089E
        dc.w    $A026        ; $0108A0
        dc.w    $6600        ; $0108A2
        dc.w    $00A4        ; $0108A4
        dc.w    $31FC        ; $0108A6
        dc.w    $0019        ; $0108A8
        dc.w    $A026        ; $0108AA
        dc.w    $6000        ; $0108AC
        dc.w    $009A        ; $0108AE
        dc.w    $31FC        ; $0108B0
        dc.w    $0039        ; $0108B2
        dc.w    $A026        ; $0108B4
        dc.w    $6000        ; $0108B6
        dc.w    $0090        ; $0108B8
        dc.w    $0801        ; $0108BA
        dc.w    $0003        ; $0108BC
        dc.w    $673A        ; $0108BE
        dc.w    $11FC        ; $0108C0
        dc.w    $0001        ; $0108C2
        dc.w    $A02C        ; $0108C4
        dc.w    $11FC        ; $0108C6
        dc.w    $0000        ; $0108C8
        dc.w    $A02D        ; $0108CA
        dc.w    $11FC        ; $0108CC
        dc.w    $00A9        ; $0108CE
        dc.w    $C8A4        ; $0108D0
        dc.w    $0C78        ; $0108D2
        dc.w    $0039        ; $0108D4
        dc.w    $A026        ; $0108D6
        dc.w    $6C18        ; $0108D8
        dc.w    $5278        ; $0108DA
        dc.w    $A026        ; $0108DC
        dc.w    $0C78        ; $0108DE
        dc.w    $001A        ; $0108E0
        dc.w    $A026        ; $0108E2
        dc.w    $6600        ; $0108E4
        dc.w    $0062        ; $0108E6
        dc.w    $31FC        ; $0108E8
        dc.w    $0034        ; $0108EA
        dc.w    $A026        ; $0108EC
        dc.w    $6000        ; $0108EE
        dc.w    $0058        ; $0108F0
        dc.w    $4278        ; $0108F2
        dc.w    $A026        ; $0108F4
        dc.w    $6000        ; $0108F6
        dc.w    $0050        ; $0108F8
        dc.w    $0801        ; $0108FA
        dc.w    $0000        ; $0108FC
        dc.w    $6722        ; $0108FE
        dc.w    $4A78        ; $010900
        dc.w    $A028        ; $010902
        dc.w    $6700        ; $010904
        dc.w    $0042        ; $010906
        dc.w    $11FC        ; $010908
        dc.w    $0001        ; $01090A
        dc.w    $A02C        ; $01090C
        dc.w    $11FC        ; $01090E
        dc.w    $0000        ; $010910
        dc.w    $A02D        ; $010912
        dc.w    $4278        ; $010914
        dc.w    $A028        ; $010916
        dc.w    $11FC        ; $010918
        dc.w    $00A9        ; $01091A
        dc.w    $C8A4        ; $01091C
        dc.w    $6000        ; $01091E
        dc.w    $0028        ; $010920
        dc.w    $0801        ; $010922
        dc.w    $0001        ; $010924
        dc.w    $6720        ; $010926
        dc.w    $4A78        ; $010928
        dc.w    $A028        ; $01092A
        dc.w    $6600        ; $01092C
        dc.w    $001A        ; $01092E
        dc.w    $11FC        ; $010930
        dc.w    $0001        ; $010932
        dc.w    $A02C        ; $010934
        dc.w    $11FC        ; $010936
        dc.w    $0000        ; $010938
        dc.w    $A02D        ; $01093A
        dc.w    $31FC        ; $01093C
        dc.w    $0001        ; $01093E
        dc.w    $A028        ; $010940
        dc.w    $11FC        ; $010942
        dc.w    $00A9        ; $010944
        dc.w    $C8A4        ; $010946
        dc.w    $3038        ; $010948
        dc.w    $A026        ; $01094A
        dc.w    $0C40        ; $01094C
        dc.w    $0019        ; $01094E
        dc.w    $6E00        ; $010950
        dc.w    $000E        ; $010952
        dc.w    $4A78        ; $010954
        dc.w    $A028        ; $010956
        dc.w    $6700        ; $010958
        dc.w    $0006        ; $01095A
        dc.w    $0640        ; $01095C
        dc.w    $001A        ; $01095E
        dc.w    $41F9        ; $010960
        dc.w    $0089        ; $010962
        dc.w    $0974        ; $010964
        dc.w    $1030        ; $010966
        dc.w    $0000        ; $010968
        dc.w    $0240        ; $01096A
        dc.w    $00FF        ; $01096C
        dc.w    $31C0        ; $01096E
        dc.w    $A024        ; $010970
        dc.w    $4E75        ; $010972
        dc.w    $4142        ; $010974
        dc.w    $4344        ; $010976
        dc.w    $4546        ; $010978
        dc.w    $4748        ; $01097A
        dc.w    $494A        ; $01097C
        dc.w    $4B4C        ; $01097E
        dc.w    $4D4E        ; $010980
        dc.w    $4F50        ; $010982
        dc.w    $5152        ; $010984
        dc.w    $5354        ; $010986
        dc.w    $5556        ; $010988
        dc.w    $5758        ; $01098A
        dc.w    $595A        ; $01098C
        dc.w    $6162        ; $01098E
        dc.w    $6364        ; $010990
        dc.w    $6566        ; $010992
        dc.w    $6768        ; $010994
        dc.w    $696A        ; $010996
        dc.w    $6B6C        ; $010998
        dc.w    $6D6E        ; $01099A
        dc.w    $6F70        ; $01099C
        dc.w    $7172        ; $01099E
        dc.w    $7374        ; $0109A0
        dc.w    $7576        ; $0109A2
        dc.w    $7778        ; $0109A4
        dc.w    $797A        ; $0109A6
        dc.w    $2E21        ; $0109A8
        dc.w    $3F20        ; $0109AA
        dc.w    $0803        ; $0109AC

; --- Early main code 2 handler ---
EarlyMC2Handler:
        MOVE.W #$002C,$00FF0008        ; $0109AE
        MOVE.W #$002C,$C87A.W        ; $0109B6
        BCLR #6,$C875.W        ; $0109BC
        MOVE.W $C874.W,(A5)        ; $0109C2
        MOVE.W #$0083,MARS_SYS_BASE        ; $0109C6
        BTST #252,$00A15181        ; $0109CE
        JSR $008826C8        ; $0109D6
        MOVE.L #$000A0907,D0        ; $0109DC
        JSR $008814BE        ; $0109E2
        MOVE.B #$0001,$C80D.W        ; $0109E8
        MOVEQ #$00,D0        ; $0109EE
        DC.W $41F8 ; Unknown        ; $0109F0
        OR.L D0,D2        ; $0109F2
        MOVEQ #$1F,D1        ; $0109F4
        MOVE.L D0,(A0)+        ; $0109F6
        DBRA D1,$008909F6        ; $0109F8
        LEA $00FF7B80,A0        ; $0109FC
        MOVEQ #$7F,D1        ; $010A02
        MOVE.L D0,(A0)+        ; $010A04
        DBRA D1,$00890A04        ; $010A06
        MOVE.L #$60000002,(A5)        ; $010A0A
        MOVE.W #$17FF,D1        ; $010A10
        MOVE.L D0,(A6)        ; $010A14
        DBRA D1,$00890A14        ; $010A16
        JSR $008849AA        ; $010A1A
        CLR.B $C880.W        ; $010A20
        CLR.B $C882.W        ; $010A24
        CLR.B $8000.W        ; $010A28
        CLR.B $8002.W        ; $010A2C
        CLR.B $A012.W        ; $010A30
        CLR $A018.W        ; $010A34
        JSR $008849AA        ; $010A38
        MOVE.L #$008BB4FC,$C96C.W        ; $010A3E
        MOVE.B #$0001,$C809.W        ; $010A46
        MOVE.B #$0001,$C80A.W        ; $010A4C
        BSET #6,$C80E.W        ; $010A52
        MOVE.B #$0001,$C802.W        ; $010A58
        MOVE.W #$0001,$A05C.W        ; $010A5E
        LEA $00FF1000,A0        ; $010A64
        MOVE.W #$037F,D0        ; $010A6A
        CLR.W (A0)+        ; $010A6E
        DBRA D0,$00890A6E        ; $010A70
        MOVE.W #$0001,D0        ; $010A74
        MOVE.W #$0001,D1        ; $010A78
        MOVE.W #$0001,D2        ; $010A7C
        MOVE.W #$0026,D3        ; $010A80
        MOVE.W #$001A,D4        ; $010A84
        LEA $00FF1000,A0        ; $010A88
        JSR $0088E22C(PC)        ; $010A8E
        LEA $00FF1000,A0        ; $010A92
        JSR $0088E2F0(PC)        ; $010A98
        JSR $0088E1BC(PC)        ; $010A9C
        BCLR #7,$00A15181        ; $010AA0
        LEA $00FF6E00,A0        ; $010AA8
        ADDA.L #$00000160,A0        ; $010AAE
        LEA $00891062,A1        ; $010AB4
        MOVE.W #$003F,D0        ; $010ABA
        MOVE.W (A1)+,D1        ; $010ABE
        BSET #15,D1        ; $010AC0
        MOVE.W D1,(A0)+        ; $010AC4
        DBRA D0,$00890ABE        ; $010AC6
        BTST #4,$C80E.W        ; $010ACA
        BEQ $00890AF6        ; $010AD0
        LEA $00FF6E00,A0        ; $010AD4
        ADDA.L #$00000180,A0        ; $010ADA
        LEA $008910E2,A1        ; $010AE0
        MOVE.W #$000F,D0        ; $010AE6
        MOVE.W (A1)+,D1        ; $010AEA
        BSET #15,D1        ; $010AEC
        MOVE.W D1,(A0)+        ; $010AF0
        DBRA D0,$00890AEA        ; $010AF2
        LEA $000EC980,A0        ; $010AF6
        MOVEA.L #$06018000,A1        ; $010AFC
        JSR $0088E316(PC)        ; $010B02
        LEA $000ECC90,A0        ; $010B06
        MOVEA.L #$06018500,A1        ; $010B0C
        JSR $0088E316(PC)        ; $010B12
        LEA $000ECE20,A0        ; $010B16
        MOVEA.L #$06018C00,A1        ; $010B1C
        JSR $0088E316(PC)        ; $010B22
        BTST #4,$C80E.W        ; $010B26
        BEQ $00890B40        ; $010B2C
        LEA $000ECAB0,A0        ; $010B30
        MOVEA.L #$06018F80,A1        ; $010B36
        JSR $0088E316(PC)        ; $010B3C
        LEA $000EBB40,A0        ; $010B40
        MOVEA.L #$0601AD00,A1        ; $010B46
        JSR $0088E316(PC)        ; $010B4C
        LEA $000EB980,A0        ; $010B50
        MOVEA.L #$0601DF00,A1        ; $010B56
        JSR $0088E316(PC)        ; $010B5C
        MOVEQ #$00,D0        ; $010B60
        MOVE.B $FEB1.W,D0        ; $010B62
        BTST #5,$C80E.W        ; $010B66
        BEQ $00890B74        ; $010B6C
        MOVE.B $FEB2.W,D0        ; $010B70
        BTST #4,$C80E.W        ; $010B74
        BEQ $00890B82        ; $010B7A
        MOVE.B $FEB3.W,D0        ; $010B7E
        ADD.L D0,D0        ; $010B82
        ADD.L D0,D0        ; $010B84
        LEA $0089103E,A0        ; $010B86
        MOVEA.L <EA:30>,A0        ; $010B8C
        BTST #124,D0        ; $010B8E
        BTST #192,D1        ; $010B92
        JSR $0088E316(PC)        ; $010B96
        MOVEQ #$00,D0        ; $010B9A
        MOVE.B $FEA5.W,D0        ; $010B9C
        BTST #5,$C80E.W        ; $010BA0
        BEQ $00890BAE        ; $010BA6
        MOVE.B $FEA7.W,D0        ; $010BAA
        BTST #4,$C80E.W        ; $010BAE
        BEQ $00890BBC        ; $010BB4
        MOVE.B $FEAB.W,D0        ; $010BB8
        ADD.L D0,D0        ; $010BBC
        ADD.L D0,D0        ; $010BBE
        LEA $0089104A,A0        ; $010BC0
        MOVEA.L <EA:30>,A0        ; $010BC6
        BTST #124,D0        ; $010BC8
        BTST #64,D1        ; $010BCC
        JSR $0088E316(PC)        ; $010BD0
        LEA $000EDE10,A0        ; $010BD4
        MOVEA.L #$06010000,A1        ; $010BDA
        JSR $0088E316(PC)        ; $010BE0
        MOVE.B #$0000,$A019.W        ; $010BE4
        MOVE.B #$0000,$A01A.W        ; $010BEA
        MOVE.B #$0002,$A01C.W        ; $010BF0
        MOVE.L #$00000000,$A022.W        ; $010BF6
        MOVE.L #$00000000,$A026.W        ; $010BFE
        MOVE.B #$0000,$A02E.W        ; $010C06
        MOVE.L #$00000000,$A032.W        ; $010C0C
        MOVE.L #$00000000,$A036.W        ; $010C14
        MOVE.B #$0000,$A03E.W        ; $010C1C
        CLR.B $A050.W        ; $010C22
        MOVE.W #$0010,$A052.W        ; $010C26
        MOVEQ #$00,D0        ; $010C2C
        MOVE.B $FEA8.W,D0        ; $010C2E
        LEA $00891102,A0        ; $010C32
        BTST #5,$C80E.W        ; $010C38
        BNE $00890C56        ; $010C3E
        MOVE.B $FEAC.W,D0        ; $010C40
        LEA $00891112,A0        ; $010C44
        BTST #4,$C80E.W        ; $010C4A
        BNE $00890C56        ; $010C50
        MOVE.B #$0000,D0        ; $010C52
        ADD.W D0,D0        ; $010C56
        ADD.W D0,D0        ; $010C58
        MOVE.L <EA:30>,$0000.W        ; $010C5A
        DC.W $A02A ; Unknown        ; $010C5E
        MOVE.L <EA:30>,$0000.W        ; $010C60
        DC.W $A03A ; Unknown        ; $010C64
        CLR.W $A046.W        ; $010C66
        DC.W $41F8 ; Unknown        ; $010C6A
        DC.W $A046 ; Unknown        ; $010C6C
        DC.W $43F8 ; Unknown        ; $010C6E
        AND.B D0,D1        ; $010C70
        MOVE.W #$0013,D2        ; $010C72
        BTST #0,D0        ; $010C76
        MOVE.B $0003(A0),D0        ; $010C7A
        MOVE.B $0003(A1),D1        ; $010C7E
        AND.B D0,D1        ; $010C82
        MOVE.B D0,$0003(A0)        ; $010C84
        MOVE.B $0002(A0),D0        ; $010C88
        MOVE.B $0002(A1),D1        ; $010C8C
        AND.B D0,D1        ; $010C90
        MOVE.B D0,D1        ; $010C92
        BTST #15,D0        ; $010C94
        MOVE.B D0,$0002(A0)        ; $010C98
        LSR.B #4,D1        ; $010C9C
        BTST #0,D0        ; $010C9E
        MOVE.B $0001(A0),D0        ; $010CA2
        AND.B D0,D1        ; $010CA6
        MOVE.B $0001(A1),D1        ; $010CA8
        AND.B D0,D1        ; $010CAC
        BCC $00890CC2        ; $010CAE
        BTST #0,D0        ; $010CB2
        MOVE.B #$0040,D1        ; $010CB6
        AND.B D0,D1        ; $010CBA
        MOVE.B #$0001,D1        ; $010CBC
        BRA $00890CDA        ; $010CC0
        CLR D1        ; $010CC2
        CMPI.B #$60,D0        ; $010CC4
        BCS $00890CDA        ; $010CC8
        BTST #0,D0        ; $010CCC
        MOVE.B #$0060,D1        ; $010CD0
        OR.B D0,D1        ; $010CD4
        MOVE.B #$0001,D1        ; $010CD6
        MOVE.B D0,$0001(A0)        ; $010CDA
        BTST #0,D0        ; $010CDE
        MOVE.B (A0),D0        ; $010CE2
        AND.B D0,D1        ; $010CE4
        MOVE.B (A1),D1        ; $010CE6
        AND.B D0,D1        ; $010CE8
        MOVE.B D0,(A0)        ; $010CEA
        SUBQ.L #4,A1        ; $010CEC
        DBRA D2,$00890C76        ; $010CEE
        TST.L $A046.W        ; $010CF2
        BNE $00890D00        ; $010CF6
        MOVE.L #$CCCC0CCC,$A046.W        ; $010CF8
        DC.W $41F8 ; Unknown        ; $010D00
        AND.B D0,D1        ; $010D02
        MOVE.W #$0013,D0        ; $010D04
        TST.L (A0)        ; $010D08
        BEQ $00890D16        ; $010D0A
        SUBQ.L #4,A0        ; $010D0E
        DBRA D0,$00890D08        ; $010D10
        BRA $00890D20        ; $010D14
        MOVE.L #$CCCC0CCC,(A0)+        ; $010D16
        DBRA D0,$00890D16        ; $010D1C
        MOVE.W #$0000,$A042.W        ; $010D20
        DC.W $41F8 ; Unknown        ; $010D26
        AND.B D0,D1        ; $010D28
        MOVE.L $0010(A0),D0        ; $010D2A
        CMPI.L #$CCCC0CCC,D0        ; $010D2E
        BNE $00890D3C        ; $010D34
        MOVE.W #$0001,$A042.W        ; $010D36
        MOVEQ #$00,D2        ; $010D3C
        MOVE.L D2,$A01E.W        ; $010D3E
        DC.W $41F8 ; Unknown        ; $010D42
        AND.B D0,D1        ; $010D44
        MOVE.L #$60000000,D0        ; $010D46
        MOVE.W #$0013,D3        ; $010D4C
        MOVE.L (A0)+,D1        ; $010D50
        BEQ $00890D66        ; $010D52
        CMPI.L #$CCCC0CCC,D1        ; $010D54
        BEQ $00890D66        ; $010D5A
        CMP.L D1,D0        ; $010D5C
        BLE $00890D66        ; $010D5E
        MOVE.L D1,D0        ; $010D60
        MOVE.L D2,$A01E.W        ; $010D62
        BCLR #0,D2        ; $010D66
        BCLR D6,D0        ; $010D6A
        DBRA D3,$00890D50        ; $010D6C
        TST.B COMM0        ; $010D70
        BNE $00890D70        ; $010D76
        MOVE.L #$06020000,COMM4        ; $010D78
        MOVE.B #$0026,$00A15121        ; $010D82
        MOVE.B #$0001,COMM0        ; $010D8A
        BTST #4,$C80E.W        ; $010D92
        BNE $00890E7E        ; $010D98
        MOVEA.L #$0601AD00,A0        ; $010D9C
        MOVEA.L #$06028000,A1        ; $010DA2
        MOVE.W #$0028,D0        ; $010DA8
        MOVE.W #$0140,D1        ; $010DAC
        MOVE.W #$00D8,D2        ; $010DB0
        JSR $00891A98(PC)        ; $010DB4
        MOVEQ #$00,D3        ; $010DB8
        MOVEQ #$00,D4        ; $010DBA
        MOVE.W #$0013,D5        ; $010DBC
        MOVEA.L #$06028030,A1        ; $010DC0
        ADDA.L D3,A1        ; $010DC6
        BCLR #0,D3        ; $010DC8
        BCLR D6,D0        ; $010DCC
        DC.W $45F8 ; Unknown        ; $010DCE
        AND.B D0,D1        ; $010DD0
        ADDA.L D4,A2        ; $010DD2
        SUBQ.L #4,D4        ; $010DD4
        MOVE.W #$00D8,D2        ; $010DD6
        JSR $00891942(PC)        ; $010DDA
        DBRA D5,$00890DC0        ; $010DDE
        CMPI.L #$61000000,$C254.W        ; $010DE2
        BEQ $00890E7A        ; $010DEA
        DC.W $45F8 ; Unknown        ; $010DEE
        AND.B D0,D1        ; $010DF0
        MOVE.L #$60000000,D1        ; $010DF2
        CLR.B D3        ; $010DF8
        MOVE.W #$0001,D2        ; $010DFA
        MOVE.W #$0013,D4        ; $010DFE
        MOVE.L (A2)+,D0        ; $010E02
        CMPI.L #$CCCC0CCC,D0        ; $010E04
        BEQ $00890E14        ; $010E0A
        CMP.L D1,D0        ; $010E0C
        BCC $00890E14        ; $010E0E
        MOVE.L D0,D1        ; $010E10
        MOVE.W D2,D3        ; $010E12
        SUBQ.W #1,D2        ; $010E14
        DBRA D4,$00890E02        ; $010E16
        TST.W D3        ; $010E1A
        BEQ $00890E54        ; $010E1C
        MOVEA.L #$06028030,A0        ; $010E1E
        DC.W $5343 ; Unknown        ; $010E24
        BCLR #0,D3        ; $010E26
        DC.W $FFFF ; Unknown        ; $010E2A
        LSL.L #7,D3        ; $010E2C
        MOVE.L D3,D4        ; $010E2E
        LSL.L #1,D3        ; $010E30
        ADD.L D3,D4        ; $010E32
        LSL.L #2,D3        ; $010E34
        ADD.L D3,D4        ; $010E36
        LSL.L #1,D3        ; $010E38
        ADD.L D4,D3        ; $010E3A
        ADDA.L D3,A0        ; $010E3C
        MOVE.W #$0078,D0        ; $010E3E
        MOVE.W #$0010,D1        ; $010E42
        MOVE.W #$0008,D2        ; $010E46
        MOVE.W #$00D8,D3        ; $010E4A
        JSR $0088E406        ; $010E4E
        MOVEA.L #$06019D00,A0        ; $010E54
        MOVEA.L #$06028088,A1        ; $010E5A
        ADDA.L $A01E.W,A1        ; $010E60
        MOVE.W #$0050,D0        ; $010E64
        MOVE.W #$0010,D1        ; $010E68
        MOVE.W #$00D8,D2        ; $010E6C
        BCLR #0,D3        ; $010E70
        BCLR D6,D0        ; $010E74
        JSR $00891A98(PC)        ; $010E76
        BRA $00890FC8        ; $010E7A
        MOVEA.L #$0601AD00,A0        ; $010E7E
        MOVEA.L #$06028000,A1        ; $010E84
        MOVE.W #$0028,D0        ; $010E8A
        MOVE.W #$0140,D1        ; $010E8E
        MOVE.W #$0080,D2        ; $010E92
        JSR $00891A98(PC)        ; $010E96
        MOVEQ #$00,D3        ; $010E9A
        MOVEQ #$00,D4        ; $010E9C
        MOVE.W #$0013,D5        ; $010E9E
        MOVEA.L #$06028030,A1        ; $010EA2
        ADDA.L D3,A1        ; $010EA8
        BCLR #0,D3        ; $010EAA
        dc.w    $0800        ; $010EAE
        ADDA.L D4,A2        ; $010EB4
        SUBQ.L #4,D4        ; $010EB6
        MOVE.W #$0080,D2        ; $010EB8
        JSR $00891942(PC)        ; $010EBC
        DBRA D5,$00890EA2        ; $010EC0
        DC.W $45F8 ; Unknown        ; $010EC4
        AND.B D0,D1        ; $010EC6
        MOVE.L #$60000000,D1        ; $010EC8
        CLR.B D3        ; $010ECE
        MOVE.W #$0001,D2        ; $010ED0
        MOVE.W #$0013,D4        ; $010ED4
        MOVE.L (A2)+,D0        ; $010ED8
        CMPI.L #$CCCC0CCC,D0        ; $010EDA
        BEQ $00890EEA        ; $010EE0
        CMP.L D1,D0        ; $010EE2
        BCC $00890EEA        ; $010EE4
        MOVE.L D0,D1        ; $010EE6
        MOVE.W D2,D3        ; $010EE8
        SUBQ.W #1,D2        ; $010EEA
        DBRA D4,$00890ED8        ; $010EEC
        TST.W D3        ; $010EF0
        BEQ $00890F1E        ; $010EF2
        MOVEA.L #$06028000,A0        ; $010EF4
        DC.W $5343 ; Unknown        ; $010EFA
        BCLR #0,D3        ; $010EFC
        DC.W $FFFF ; Unknown        ; $010F00
        LSL.L #8,D3        ; $010F02
        LSL.L #3,D3        ; $010F04
        ADDA.L D3,A0        ; $010F06
        MOVE.W #$0080,D0        ; $010F08
        MOVE.W #$0010,D1        ; $010F0C
        MOVE.W #$0008,D2        ; $010F10
        MOVE.W #$0080,D3        ; $010F14
        JSR $0088E406        ; $010F18
        BSR $00891B6A        ; $010F1E
        MOVEA.L #$2601AD00,A0        ; $010F22
        MOVEA.L #$26032000,A1        ; $010F28
        MOVE.W #$0028,D0        ; $010F2E
        MOVE.W #$0140,D1        ; $010F32
        MOVE.W #$0080,D2        ; $010F36
        JSR $00891A98(PC)        ; $010F3A
        MOVEQ #$00,D3        ; $010F3E
        MOVEQ #$00,D4        ; $010F40
        MOVE.W #$0013,D5        ; $010F42
        MOVEA.L #$06032030,A1        ; $010F46
        ADDA.L D3,A1        ; $010F4C
        BCLR #0,D3        ; $010F4E
        dc.w    $0800        ; $010F52
        ADDA.L D4,A2        ; $010F58
        SUBQ.L #4,D4        ; $010F5A
        MOVE.W #$0080,D2        ; $010F5C
        JSR $00891942(PC)        ; $010F60
        DBRA D5,$00890F46        ; $010F64
        DC.W $45F8 ; Unknown        ; $010F68
        AND.B D0,D1        ; $010F6A
        MOVE.L #$60000000,D1        ; $010F6C
        CLR.B D3        ; $010F72
        MOVE.W #$0001,D2        ; $010F74
        MOVE.W #$0013,D4        ; $010F78
        MOVE.L (A2)+,D0        ; $010F7C
        CMPI.L #$CCCC0CCC,D0        ; $010F7E
        BEQ $00890F8E        ; $010F84
        CMP.L D1,D0        ; $010F86
        BCC $00890F8E        ; $010F88
        MOVE.L D0,D1        ; $010F8A
        MOVE.W D2,D3        ; $010F8C
        SUBQ.W #1,D2        ; $010F8E
        DBRA D4,$00890F7C        ; $010F90
        TST.W D3        ; $010F94
        BEQ $00890FC2        ; $010F96
        MOVEA.L #$06032000,A0        ; $010F98
        DC.W $5343 ; Unknown        ; $010F9E
        BCLR #0,D3        ; $010FA0
        DC.W $FFFF ; Unknown        ; $010FA4
        LSL.L #8,D3        ; $010FA6
        LSL.L #3,D3        ; $010FA8
        ADDA.L D3,A0        ; $010FAA
        MOVE.W #$0080,D0        ; $010FAC
        MOVE.W #$0010,D1        ; $010FB0
        MOVE.W #$0008,D2        ; $010FB4
        MOVE.W #$0080,D3        ; $010FB8
        JSR $0088E406        ; $010FBC
        JSR $0088204A        ; $010FC2
        MOVE.B #$0001,$C821.W        ; $010FC8
        BTST #252,$00A15181        ; $010FCE
        BTST #1,$00A15181        ; $010FD6
        MOVE.W #$8083,MARS_SYS_BASE        ; $010FDE
        BSET #6,$C875.W        ; $010FE6
        MOVE.W $C874.W,(A5)        ; $010FEC
        MOVE.W #$0018,$00FF0008        ; $010FF0
        JSR $00884998        ; $010FF8
        MOVE.W #$0000,$C87E.W        ; $010FFE
        MOVE.L #$00891166,$00FF0002        ; $011004
        BTST #4,$C80E.W        ; $01100E
        BNE $00891036        ; $011014
        MOVE.L #$00891142,$00FF0002        ; $011018
        BTST #5,$C80E.W        ; $011022
        BNE $00891036        ; $011028
        MOVE.L #$00891122,$00FF0002        ; $01102C
        MOVE.B #$008E,$C8A5.W        ; $011036
        RTS        ; $01103C
        dc.w    $000E        ; $01103E
        dc.w    $D310        ; $011040
        dc.w    $000E        ; $011042
        dc.w    $D440        ; $011044
        dc.w    $000E        ; $011046
        dc.w    $D530        ; $011048
        dc.w    $000E        ; $01104A
        dc.w    $D670        ; $01104C
        dc.w    $000E        ; $01104E
        dc.w    $D7D0        ; $011050
        dc.w    $000E        ; $011052
        dc.w    $D930        ; $011054
        dc.w    $000E        ; $011056
        dc.w    $DA70        ; $011058
        dc.w    $000E        ; $01105A
        dc.w    $DBA0        ; $01105C
        dc.w    $000E        ; $01105E
        dc.w    $DD10        ; $011060
        dc.w    $4400        ; $011062
        dc.w    $44A3        ; $011064
        dc.w    $4946        ; $011066
        dc.w    $4DE9        ; $011068
        dc.w    $1C00        ; $01106A
        dc.w    $28A3        ; $01106C
        dc.w    $3546        ; $01106E
        dc.w    $41E9        ; $011070
        dc.w    $0010        ; $011072
        dc.w    $14AF        ; $011074
        dc.w    $294E        ; $011076
        dc.w    $3DED        ; $011078
        dc.w    $1C00        ; $01107A
        dc.w    $28A3        ; $01107C
        dc.w    $3546        ; $01107E
        dc.w    $41E9        ; $011080
        dc.w    $4400        ; $011082
        dc.w    $44A3        ; $011084
        dc.w    $4946        ; $011086
        dc.w    $4DE9        ; $011088
        dc.w    $7FFF        ; $01108A
        dc.w    $63F5        ; $01108C
        dc.w    $7FFF        ; $01108E
        dc.w    $7FFF        ; $011090
        dc.w    $0010        ; $011092
        dc.w    $14AF        ; $011094
        dc.w    $294E        ; $011096
        dc.w    $3DED        ; $011098
        dc.w    $7FFF        ; $01109A
        dc.w    $7FFF        ; $01109C
        dc.w    $7FFF        ; $01109E
        dc.w    $7FFF        ; $0110A0
        dc.w    $7FFF        ; $0110A2
        dc.w    $7FFF        ; $0110A4
        dc.w    $7FFF        ; $0110A6
        dc.w    $7FFF        ; $0110A8
        dc.w    $4445        ; $0110AA
        dc.w    $512B        ; $0110AC
        dc.w    $6212        ; $0110AE
        dc.w    $6EF8        ; $0110B0
        dc.w    $7FFF        ; $0110B2
        dc.w    $0E9A        ; $0110B4
        dc.w    $7FFF        ; $0110B6
        dc.w    $7FFF        ; $0110B8
        dc.w    $7FFF        ; $0110BA
        dc.w    $7FFF        ; $0110BC
        dc.w    $7FFF        ; $0110BE
        dc.w    $7FFF        ; $0110C0
        dc.w    $0010        ; $0110C2
        dc.w    $14AF        ; $0110C4
        dc.w    $294E        ; $0110C6
        dc.w    $3DED        ; $0110C8
        dc.w    $4445        ; $0110CA
        dc.w    $512B        ; $0110CC
        dc.w    $6212        ; $0110CE
        dc.w    $6EF8        ; $0110D0
        dc.w    $7FFF        ; $0110D2
        dc.w    $031F        ; $0110D4
        dc.w    $7FFF        ; $0110D6
        dc.w    $7FFF        ; $0110D8
        dc.w    $7FFF        ; $0110DA
        dc.w    $7FFF        ; $0110DC
        dc.w    $7FFF        ; $0110DE
        dc.w    $7FFF        ; $0110E0
        dc.w    $4400        ; $0110E2
        dc.w    $44A3        ; $0110E4
        dc.w    $4946        ; $0110E6
        dc.w    $4DE9        ; $0110E8
        dc.w    $4445        ; $0110EA
        dc.w    $44C6        ; $0110EC
        dc.w    $4968        ; $0110EE
        dc.w    $4DEA        ; $0110F0
        dc.w    $7FFF        ; $0110F2
        dc.w    $63F5        ; $0110F4
        dc.w    $7FFF        ; $0110F6
        dc.w    $7FFF        ; $0110F8
        dc.w    $7FFF        ; $0110FA
        dc.w    $7FFF        ; $0110FC
        dc.w    $7FFF        ; $0110FE
        dc.w    $7FFF        ; $011100
        dc.w    $0000        ; $011102
        dc.w    $0000        ; $011104
        dc.w    $0000        ; $011106
        dc.w    $4380        ; $011108
        dc.w    $0000        ; $01110A
        dc.w    $8700        ; $01110C
        dc.w    $0000        ; $01110E
        dc.w    $CA80        ; $011110
        dc.w    $0000        ; $011112
        dc.w    $0000        ; $011114
        dc.w    $0000        ; $011116
        dc.w    $2800        ; $011118
        dc.w    $0000        ; $01111A
        dc.w    $5000        ; $01111C
        dc.w    $0000        ; $01111E
        dc.w    $7800        ; $011120
        dc.w    $4EB9        ; $011122
        dc.w    $0088        ; $011124
        dc.w    $2080        ; $011126
        dc.w    $3038        ; $011128
        dc.w    $C87E        ; $01112A
        dc.w    $227B        ; $01112C
        dc.w    $0004        ; $01112E
        dc.w    $4ED1        ; $011130
        dc.w    $0089        ; $011132
        dc.w    $11A4        ; $011134
        dc.w    $0089        ; $011136
        dc.w    $11A4        ; $011138
        dc.w    $0089        ; $01113A
        dc.w    $1240        ; $01113C
        dc.w    $0089        ; $01113E
        dc.w    $17F4        ; $011140
        dc.w    $4EB9        ; $011142
        dc.w    $0088        ; $011144
        dc.w    $2080        ; $011146
        dc.w    $3038        ; $011148
        dc.w    $C87E        ; $01114A
        dc.w    $227B        ; $01114C
        dc.w    $0004        ; $01114E
        dc.w    $4ED1        ; $011150
        dc.w    $0089        ; $011152
        dc.w    $11A4        ; $011154
        dc.w    $0089        ; $011156
        dc.w    $11A4        ; $011158
        dc.w    $0089        ; $01115A
        dc.w    $141A        ; $01115C
        dc.w    $0089        ; $01115E
        dc.w    $146E        ; $011160
        dc.w    $0089        ; $011162
        dc.w    $1862        ; $011164
        dc.w    $4EB9        ; $011166
        dc.w    $0088        ; $011168
        dc.w    $2080        ; $01116A
        dc.w    $3038        ; $01116C
        dc.w    $C87E        ; $01116E
        dc.w    $227B        ; $011170
        dc.w    $0004        ; $011172
        dc.w    $4ED1        ; $011174
        dc.w    $0089        ; $011176
        dc.w    $11A4        ; $011178
        dc.w    $0089        ; $01117A
        dc.w    $11A4        ; $01117C
        dc.w    $0089        ; $01117E
        dc.w    $11B6        ; $011180
        dc.w    $0089        ; $011182
        dc.w    $11B6        ; $011184
        dc.w    $0089        ; $011186
        dc.w    $15A8        ; $011188
        dc.w    $0089        ; $01118A
        dc.w    $1630        ; $01118C
        dc.w    $0089        ; $01118E
        dc.w    $188A        ; $011190
        dc.w    $4EBA        ; $011192
        dc.w    $A4F0        ; $011194
        dc.w    $0838        ; $011196
        dc.w    $0006        ; $011198
        dc.w    $C80E        ; $01119A
        dc.w    $6604        ; $01119C
        dc.w    $5878        ; $01119E
        dc.w    $C87E        ; $0111A0
        dc.w    $4E75        ; $0111A2
        dc.w    $4EBA        ; $0111A4
        dc.w    $0962        ; $0111A6
        dc.w    $5878        ; $0111A8
        dc.w    $C87E        ; $0111AA
        dc.w    $33FC        ; $0111AC
        dc.w    $0018        ; $0111AE
        dc.w    $00FF        ; $0111B0
        dc.w    $0008        ; $0111B2
        dc.w    $4E75        ; $0111B4
        dc.w    $207C        ; $0111B6
        dc.w    $0601        ; $0111B8
        dc.w    $8F80        ; $0111BA
        dc.w    $227C        ; $0111BC
        dc.w    $0400        ; $0111BE
        dc.w    $D018        ; $0111C0
        dc.w    $303C        ; $0111C2
        dc.w    $0078        ; $0111C4
        dc.w    $323C        ; $0111C6
        dc.w    $0018        ; $0111C8
        dc.w    $4EBA        ; $0111CA
        dc.w    $D18E        ; $0111CC
        dc.w    $207C        ; $0111CE
        dc.w    $0601        ; $0111D0
        dc.w    $9AC0        ; $0111D2
        dc.w    $227C        ; $0111D4
        dc.w    $0400        ; $0111D6
        dc.w    $D0A0        ; $0111D8
        dc.w    $303C        ; $0111DA
        dc.w    $0078        ; $0111DC
        dc.w    $323C        ; $0111DE
        dc.w    $0018        ; $0111E0
        dc.w    $4EBA        ; $0111E2
        dc.w    $D176        ; $0111E4
        dc.w    $43F9        ; $0111E6
        dc.w    $0403        ; $0111E8
        dc.w    $B048        ; $0111EA
        dc.w    $45F8        ; $0111EC
        dc.w    $A046        ; $0111EE
        dc.w    $4EBA        ; $0111F0
        dc.w    $06E2        ; $0111F2
        dc.w    $43F9        ; $0111F4
        dc.w    $0403        ; $0111F6
        dc.w    $B0D0        ; $0111F8
        dc.w    $45F8        ; $0111FA
        dc.w    $A04A        ; $0111FC
        dc.w    $4EBA        ; $0111FE
        dc.w    $06D4        ; $011200
        dc.w    $207C        ; $011202
        dc.w    $0601        ; $011204
        dc.w    $8C00        ; $011206
        dc.w    $227C        ; $011208
        dc.w    $0401        ; $01120A
        dc.w    $B010        ; $01120C
        dc.w    $303C        ; $01120E
        dc.w    $0038        ; $011210
        dc.w    $323C        ; $011212
        dc.w    $0010        ; $011214
        dc.w    $4EBA        ; $011216
        dc.w    $D142        ; $011218
        dc.w    $207C        ; $01121A
        dc.w    $0601        ; $01121C
        dc.w    $8C00        ; $01121E
        dc.w    $227C        ; $011220
        dc.w    $0401        ; $011222
        dc.w    $B098        ; $011224
        dc.w    $303C        ; $011226
        dc.w    $0038        ; $011228
        dc.w    $323C        ; $01122A
        dc.w    $0010        ; $01122C
        dc.w    $4EBA        ; $01122E
        dc.w    $D12A        ; $011230
        dc.w    $5878        ; $011232
        dc.w    $C87E        ; $011234
        dc.w    $33FC        ; $011236
        dc.w    $0018        ; $011238
        dc.w    $00FF        ; $01123A
        dc.w    $0008        ; $01123C
        dc.w    $4E75        ; $01123E
        dc.w    $4240        ; $011240
        dc.w    $4EBA        ; $011242
        dc.w    $D2E8        ; $011244
        dc.w    $4EBA        ; $011246
        dc.w    $A43C        ; $011248
        dc.w    $4EBA        ; $01124A
        dc.w    $A48E        ; $01124C
        dc.w    $207C        ; $01124E
        dc.w    $0601        ; $011250
        dc.w    $8F80        ; $011252
        dc.w    $227C        ; $011254
        dc.w    $0400        ; $011256
        dc.w    $E038        ; $011258
        dc.w    $303C        ; $01125A
        dc.w    $00D8        ; $01125C
        dc.w    $323C        ; $01125E
        dc.w    $0010        ; $011260
        dc.w    $4EBA        ; $011262
        dc.w    $D0F6        ; $011264
        dc.w    $207C        ; $011266
        dc.w    $0602        ; $011268
        dc.w    $8000        ; $01126A
        dc.w    $227C        ; $01126C
        dc.w    $0401        ; $01126E
        dc.w    $0038        ; $011270
        dc.w    $303C        ; $011272
        dc.w    $00D8        ; $011274
        dc.w    $323C        ; $011276
        dc.w    $0050        ; $011278
        dc.w    $4EBA        ; $01127A
        dc.w    $D0DE        ; $01127C
        dc.w    $43F9        ; $01127E
        dc.w    $0402        ; $011280
        dc.w    $C090        ; $011282
        dc.w    $45F8        ; $011284
        dc.w    $A046        ; $011286
        dc.w    $4EBA        ; $011288
        dc.w    $064A        ; $01128A
        dc.w    $207C        ; $01128C
        dc.w    $0601        ; $01128E
        dc.w    $8C00        ; $011290
        dc.w    $227C        ; $011292
        dc.w    $0400        ; $011294
        dc.w    $C048        ; $011296
        dc.w    $303C        ; $011298
        dc.w    $0038        ; $01129A
        dc.w    $323C        ; $01129C
        dc.w    $0010        ; $01129E
        dc.w    $4EBA        ; $0112A0
        dc.w    $D0B8        ; $0112A2
        dc.w    $4A78        ; $0112A4
        dc.w    $A042        ; $0112A6
        dc.w    $6600        ; $0112A8
        dc.w    $004A        ; $0112AA
        dc.w    $207C        ; $0112AC
        dc.w    $0601        ; $0112AE
        dc.w    $A200        ; $0112B0
        dc.w    $227C        ; $0112B2
        dc.w    $0401        ; $0112B4
        dc.w    $B030        ; $0112B6
        dc.w    $303C        ; $0112B8
        dc.w    $0080        ; $0112BA
        dc.w    $323C        ; $0112BC
        dc.w    $0010        ; $0112BE
        dc.w    $4EBA        ; $0112C0
        dc.w    $D098        ; $0112C2
        dc.w    $207C        ; $0112C4
        dc.w    $0601        ; $0112C6
        dc.w    $AA00        ; $0112C8
        dc.w    $227C        ; $0112CA
        dc.w    $0401        ; $0112CC
        dc.w    $B0D0        ; $0112CE
        dc.w    $303C        ; $0112D0
        dc.w    $0018        ; $0112D2
        dc.w    $323C        ; $0112D4
        dc.w    $0010        ; $0112D6
        dc.w    $4EBA        ; $0112D8
        dc.w    $D080        ; $0112DA
        dc.w    $207C        ; $0112DC
        dc.w    $0601        ; $0112DE
        dc.w    $AB80        ; $0112E0
        dc.w    $227C        ; $0112E2
        dc.w    $0401        ; $0112E4
        dc.w    $B100        ; $0112E6
        dc.w    $303C        ; $0112E8
        dc.w    $0018        ; $0112EA
        dc.w    $323C        ; $0112EC
        dc.w    $0010        ; $0112EE
        dc.w    $4EBA        ; $0112F0
        dc.w    $D068        ; $0112F2
        dc.w    $0C78        ; $0112F4
        dc.w    $0001        ; $0112F6
        dc.w    $A05C        ; $0112F8
        dc.w    $6700        ; $0112FA
        dc.w    $00E0        ; $0112FC
        dc.w    $0C78        ; $0112FE
        dc.w    $0002        ; $011300
        dc.w    $A05C        ; $011302
        dc.w    $6700        ; $011304
        dc.w    $00EC        ; $011306
        dc.w    $4EB9        ; $011308
        dc.w    $0088        ; $01130A
        dc.w    $179E        ; $01130C
        dc.w    $3238        ; $01130E
        dc.w    $C86C        ; $011310
        dc.w    $3401        ; $011312
        dc.w    $0202        ; $011314
        dc.w    $00E0        ; $011316
        dc.w    $672E        ; $011318
        dc.w    $11FC        ; $01131A
        dc.w    $00A8        ; $01131C
        dc.w    $C8A4        ; $01131E
        dc.w    $11FC        ; $011320
        dc.w    $0001        ; $011322
        dc.w    $C809        ; $011324
        dc.w    $11FC        ; $011326
        dc.w    $0001        ; $011328
        dc.w    $C80A        ; $01132A
        dc.w    $08F8        ; $01132C
        dc.w    $0007        ; $01132E
        dc.w    $C80E        ; $011330
        dc.w    $11FC        ; $011332
        dc.w    $0001        ; $011334
        dc.w    $C802        ; $011336
        dc.w    $4EB9        ; $011338
        dc.w    $0088        ; $01133A
        dc.w    $205E        ; $01133C
        dc.w    $31FC        ; $01133E
        dc.w    $0002        ; $011340
        dc.w    $A05C        ; $011342
        dc.w    $6000        ; $011344
        dc.w    $00C6        ; $011346
        dc.w    $0801        ; $011348
        dc.w    $0004        ; $01134A
        dc.w    $674E        ; $01134C
        dc.w    $4A38        ; $01134E
        dc.w    $A019        ; $011350
        dc.w    $6600        ; $011352
        dc.w    $001A        ; $011354
        dc.w    $4A78        ; $011356
        dc.w    $A042        ; $011358
        dc.w    $6600        ; $01135A
        dc.w    $0012        ; $01135C
        dc.w    $11FC        ; $01135E
        dc.w    $00A9        ; $011360
        dc.w    $C8A4        ; $011362
        dc.w    $11FC        ; $011364
        dc.w    $0001        ; $011366
        dc.w    $A019        ; $011368
        dc.w    $6000        ; $01136A
        dc.w    $00A0        ; $01136C
        dc.w    $11FC        ; $01136E
        dc.w    $00A8        ; $011370
        dc.w    $C8A4        ; $011372
        dc.w    $11FC        ; $011374
        dc.w    $0001        ; $011376
        dc.w    $C809        ; $011378
        dc.w    $11FC        ; $01137A
        dc.w    $0001        ; $01137C
        dc.w    $C80A        ; $01137E
        dc.w    $08F8        ; $011380
        dc.w    $0007        ; $011382
        dc.w    $C80E        ; $011384
        dc.w    $11FC        ; $011386
        dc.w    $0001        ; $011388
        dc.w    $C802        ; $01138A
        dc.w    $4EB9        ; $01138C
        dc.w    $0088        ; $01138E
        dc.w    $205E        ; $011390
        dc.w    $31FC        ; $011392
        dc.w    $0002        ; $011394
        dc.w    $A05C        ; $011396
        dc.w    $6000        ; $011398
        dc.w    $0072        ; $01139A
        dc.w    $4A78        ; $01139C
        dc.w    $A042        ; $01139E
        dc.w    $666A        ; $0113A0
        dc.w    $0801        ; $0113A2
        dc.w    $0002        ; $0113A4
        dc.w    $6716        ; $0113A6
        dc.w    $4A38        ; $0113A8
        dc.w    $A019        ; $0113AA
        dc.w    $6700        ; $0113AC
        dc.w    $005E        ; $0113AE
        dc.w    $11FC        ; $0113B0
        dc.w    $00A9        ; $0113B2
        dc.w    $C8A4        ; $0113B4
        dc.w    $4238        ; $0113B6
        dc.w    $A019        ; $0113B8
        dc.w    $6000        ; $0113BA
        dc.w    $0050        ; $0113BC
        dc.w    $0801        ; $0113BE
        dc.w    $0003        ; $0113C0
        dc.w    $6748        ; $0113C2
        dc.w    $4A38        ; $0113C4
        dc.w    $A019        ; $0113C6
        dc.w    $6600        ; $0113C8
        dc.w    $0042        ; $0113CA
        dc.w    $11FC        ; $0113CC
        dc.w    $00A9        ; $0113CE
        dc.w    $C8A4        ; $0113D0
        dc.w    $11FC        ; $0113D2
        dc.w    $0001        ; $0113D4
        dc.w    $A019        ; $0113D6
        dc.w    $6000        ; $0113D8
        dc.w    $0032        ; $0113DA
        dc.w    $4EB9        ; $0113DC
        dc.w    $0088        ; $0113DE
        dc.w    $FB36        ; $0113E0
        dc.w    $0838        ; $0113E2
        dc.w    $0006        ; $0113E4
        dc.w    $C80E        ; $0113E6
        dc.w    $6622        ; $0113E8
        dc.w    $4278        ; $0113EA
        dc.w    $A05C        ; $0113EC
        dc.w    $6000        ; $0113EE
        dc.w    $001C        ; $0113F0
        dc.w    $4EB9        ; $0113F2
        dc.w    $0088        ; $0113F4
        dc.w    $FB36        ; $0113F6
        dc.w    $0838        ; $0113F8
        dc.w    $0007        ; $0113FA
        dc.w    $C80E        ; $0113FC
        dc.w    $660C        ; $0113FE
        dc.w    $4278        ; $011400
        dc.w    $A05C        ; $011402
        dc.w    $5878        ; $011404
        dc.w    $C87E        ; $011406
        dc.w    $6000        ; $011408
        dc.w    $0006        ; $01140A
        dc.w    $6100        ; $01140C
        dc.w    $05AA        ; $01140E
        dc.w    $33FC        ; $011410
        dc.w    $0018        ; $011412
        dc.w    $00FF        ; $011414
        dc.w    $0008        ; $011416
        dc.w    $4E75        ; $011418
        dc.w    $4240        ; $01141A
        dc.w    $4EBA        ; $01141C
        dc.w    $D10E        ; $01141E
        dc.w    $4A39        ; $011420
        dc.w    $00A1        ; $011422
        dc.w    $5120        ; $011424
        dc.w    $66F8        ; $011426
        dc.w    $33FC        ; $011428
        dc.w    $0101        ; $01142A
        dc.w    $00A1        ; $01142C
        dc.w    $512C        ; $01142E
        dc.w    $33FC        ; $011430
        dc.w    $8000        ; $011432
        dc.w    $00A1        ; $011434
        dc.w    $5128        ; $011436
        dc.w    $13FC        ; $011438
        dc.w    $002C        ; $01143A
        dc.w    $00A1        ; $01143C
        dc.w    $5121        ; $01143E
        dc.w    $13FC        ; $011440
        dc.w    $0001        ; $011442
        dc.w    $00A1        ; $011444
        dc.w    $5120        ; $011446
        dc.w    $4A39        ; $011448
        dc.w    $00A1        ; $01144A
        dc.w    $512C        ; $01144C
        dc.w    $66F8        ; $01144E
        dc.w    $33FC        ; $011450
        dc.w    $0050        ; $011452
        dc.w    $00A1        ; $011454
        dc.w    $5128        ; $011456
        dc.w    $33FC        ; $011458
        dc.w    $0101        ; $01145A
        dc.w    $00A1        ; $01145C
        dc.w    $512C        ; $01145E
        dc.w    $33FC        ; $011460
        dc.w    $0020        ; $011462
        dc.w    $00FF        ; $011464
        dc.w    $0008        ; $011466
        dc.w    $5878        ; $011468
        dc.w    $C87E        ; $01146A
        dc.w    $4E75        ; $01146C
        dc.w    $4240        ; $01146E
        dc.w    $4EBA        ; $011470
        dc.w    $D0BA        ; $011472
        dc.w    $4EBA        ; $011474
        dc.w    $A20E        ; $011476
        dc.w    $4EBA        ; $011478
        dc.w    $A260        ; $01147A
        dc.w    $207C        ; $01147C
        dc.w    $0601        ; $01147E
        dc.w    $8F80        ; $011480
        dc.w    $227C        ; $011482
        dc.w    $0400        ; $011484
        dc.w    $E038        ; $011486
        dc.w    $303C        ; $011488
        dc.w    $00D8        ; $01148A
        dc.w    $323C        ; $01148C
        dc.w    $0010        ; $01148E
        dc.w    $4EBA        ; $011490
        dc.w    $CEC8        ; $011492
        dc.w    $207C        ; $011494
        dc.w    $2602        ; $011496
        dc.w    $8000        ; $011498
        dc.w    $2038        ; $01149A
        dc.w    $A022        ; $01149C
        dc.w    $D1C0        ; $01149E
        dc.w    $227C        ; $0114A0
        dc.w    $2401        ; $0114A2
        dc.w    $0038        ; $0114A4
        dc.w    $303C        ; $0114A6
        dc.w    $00D8        ; $0114A8
        dc.w    $323C        ; $0114AA
        dc.w    $0050        ; $0114AC
        dc.w    $4EBA        ; $0114AE
        dc.w    $CEAA        ; $0114B0
        dc.w    $4AB8        ; $0114B2
        dc.w    $A026        ; $0114B4
        dc.w    $6700        ; $0114B6
        dc.w    $001C        ; $0114B8
        dc.w    $2038        ; $0114BA
        dc.w    $A022        ; $0114BC
        dc.w    $2238        ; $0114BE
        dc.w    $A026        ; $0114C0
        dc.w    $D081        ; $0114C2
        dc.w    $21C0        ; $0114C4
        dc.w    $A022        ; $0114C6
        dc.w    $5338        ; $0114C8
        dc.w    $A02E        ; $0114CA
        dc.w    $6400        ; $0114CC
        dc.w    $00CC        ; $0114CE
        dc.w    $42B8        ; $0114D0
        dc.w    $A026        ; $0114D2
        dc.w    $0C78        ; $0114D4
        dc.w    $0001        ; $0114D6
        dc.w    $A05C        ; $0114D8
        dc.w    $6700        ; $0114DA
        dc.w    $008E        ; $0114DC
        dc.w    $0C78        ; $0114DE
        dc.w    $0002        ; $0114E0
        dc.w    $A05C        ; $0114E2
        dc.w    $6700        ; $0114E4
        dc.w    $009A        ; $0114E6
        dc.w    $4EB9        ; $0114E8
        dc.w    $0088        ; $0114EA
        dc.w    $179E        ; $0114EC
        dc.w    $3238        ; $0114EE
        dc.w    $C86C        ; $0114F0
        dc.w    $3401        ; $0114F2
        dc.w    $0202        ; $0114F4
        dc.w    $00F0        ; $0114F6
        dc.w    $672E        ; $0114F8
        dc.w    $11FC        ; $0114FA
        dc.w    $00A8        ; $0114FC
        dc.w    $C8A4        ; $0114FE
        dc.w    $31FC        ; $011500
        dc.w    $0002        ; $011502
        dc.w    $A05C        ; $011504
        dc.w    $11FC        ; $011506
        dc.w    $0001        ; $011508
        dc.w    $C809        ; $01150A
        dc.w    $11FC        ; $01150C
        dc.w    $0001        ; $01150E
        dc.w    $C80A        ; $011510
        dc.w    $08F8        ; $011512
        dc.w    $0007        ; $011514
        dc.w    $C80E        ; $011516
        dc.w    $11FC        ; $011518
        dc.w    $0001        ; $01151A
        dc.w    $C802        ; $01151C
        dc.w    $4EB9        ; $01151E
        dc.w    $0088        ; $011520
        dc.w    $205E        ; $011522
        dc.w    $6000        ; $011524
        dc.w    $0078        ; $011526
        dc.w    $E049        ; $011528
        dc.w    $0801        ; $01152A
        dc.w    $0000        ; $01152C
        dc.w    $671A        ; $01152E
        dc.w    $4AB8        ; $011530
        dc.w    $A022        ; $011532
        dc.w    $6F00        ; $011534
        dc.w    $0064        ; $011536
        dc.w    $21FC        ; $011538
        dc.w    $FFFF        ; $01153A
        dc.w    $FE50        ; $01153C
        dc.w    $A026        ; $01153E
        dc.w    $11FC        ; $011540
        dc.w    $0007        ; $011542
        dc.w    $A02E        ; $011544
        dc.w    $6000        ; $011546
        dc.w    $0052        ; $011548
        dc.w    $0801        ; $01154A
        dc.w    $0001        ; $01154C
        dc.w    $674A        ; $01154E
        dc.w    $2038        ; $011550
        dc.w    $A022        ; $011552
        dc.w    $B0B8        ; $011554
        dc.w    $A02A        ; $011556
        dc.w    $6C00        ; $011558
        dc.w    $0040        ; $01155A
        dc.w    $21FC        ; $01155C
        dc.w    $0000        ; $01155E
        dc.w    $01B0        ; $011560
        dc.w    $A026        ; $011562
        dc.w    $11FC        ; $011564
        dc.w    $0007        ; $011566
        dc.w    $A02E        ; $011568
        dc.w    $4EB9        ; $01156A
        dc.w    $0088        ; $01156C
        dc.w    $FB36        ; $01156E
        dc.w    $0838        ; $011570
        dc.w    $0006        ; $011572
        dc.w    $C80E        ; $011574
        dc.w    $6622        ; $011576
        dc.w    $4278        ; $011578
        dc.w    $A05C        ; $01157A
        dc.w    $6000        ; $01157C
        dc.w    $001C        ; $01157E
        dc.w    $4EB9        ; $011580
        dc.w    $0088        ; $011582
        dc.w    $FB36        ; $011584
        dc.w    $0838        ; $011586
        dc.w    $0007        ; $011588
        dc.w    $C80E        ; $01158A
        dc.w    $660C        ; $01158C
        dc.w    $4278        ; $01158E
        dc.w    $A05C        ; $011590
        dc.w    $5878        ; $011592
        dc.w    $C87E        ; $011594
        dc.w    $6000        ; $011596
        dc.w    $0006        ; $011598
        dc.w    $5978        ; $01159A
        dc.w    $C87E        ; $01159C
        dc.w    $33FC        ; $01159E
        dc.w    $0018        ; $0115A0
        dc.w    $00FF        ; $0115A2
        dc.w    $0008        ; $0115A4
        dc.w    $4E75        ; $0115A6
        dc.w    $4240        ; $0115A8
        dc.w    $4EBA        ; $0115AA
        dc.w    $CF80        ; $0115AC
        dc.w    $4A39        ; $0115AE
        dc.w    $00A1        ; $0115B0
        dc.w    $5120        ; $0115B2
        dc.w    $66F8        ; $0115B4
        dc.w    $33FC        ; $0115B6
        dc.w    $0101        ; $0115B8
        dc.w    $00A1        ; $0115BA
        dc.w    $512C        ; $0115BC
        dc.w    $33FC        ; $0115BE
        dc.w    $8000        ; $0115C0
        dc.w    $00A1        ; $0115C2
        dc.w    $5128        ; $0115C4
        dc.w    $13FC        ; $0115C6
        dc.w    $002C        ; $0115C8
        dc.w    $00A1        ; $0115CA
        dc.w    $5121        ; $0115CC
        dc.w    $13FC        ; $0115CE
        dc.w    $0001        ; $0115D0
        dc.w    $00A1        ; $0115D2
        dc.w    $5120        ; $0115D4
        dc.w    $4A39        ; $0115D6
        dc.w    $00A1        ; $0115D8
        dc.w    $512C        ; $0115DA
        dc.w    $66F8        ; $0115DC
        dc.w    $33FC        ; $0115DE
        dc.w    $0050        ; $0115E0
        dc.w    $00A1        ; $0115E2
        dc.w    $5128        ; $0115E4
        dc.w    $33FC        ; $0115E6
        dc.w    $0101        ; $0115E8
        dc.w    $00A1        ; $0115EA
        dc.w    $512C        ; $0115EC
        dc.w    $207C        ; $0115EE
        dc.w    $2602        ; $0115F0
        dc.w    $8000        ; $0115F2
        dc.w    $2038        ; $0115F4
        dc.w    $A022        ; $0115F6
        dc.w    $D1C0        ; $0115F8
        dc.w    $227C        ; $0115FA
        dc.w    $2401        ; $0115FC
        dc.w    $0018        ; $0115FE
        dc.w    $303C        ; $011600
        dc.w    $0080        ; $011602
        dc.w    $323C        ; $011604
        dc.w    $0050        ; $011606
        dc.w    $4EBA        ; $011608
        dc.w    $CD50        ; $01160A
        dc.w    $5378        ; $01160C
        dc.w    $A052        ; $01160E
        dc.w    $640C        ; $011610
        dc.w    $31FC        ; $011612
        dc.w    $0010        ; $011614
        dc.w    $A052        ; $011616
        dc.w    $0878        ; $011618
        dc.w    $0000        ; $01161A
        dc.w    $A050        ; $01161C
        dc.w    $6100        ; $01161E
        dc.w    $065E        ; $011620
        dc.w    $33FC        ; $011622
        dc.w    $0020        ; $011624
        dc.w    $00FF        ; $011626
        dc.w    $0008        ; $011628
        dc.w    $5878        ; $01162A
        dc.w    $C87E        ; $01162C
        dc.w    $4E75        ; $01162E
        dc.w    $4240        ; $011630
        dc.w    $4EBA        ; $011632
        dc.w    $CEF8        ; $011634
        dc.w    $4EBA        ; $011636
        dc.w    $A04C        ; $011638
        dc.w    $4EBA        ; $01163A
        dc.w    $A09E        ; $01163C
        dc.w    $207C        ; $01163E
        dc.w    $2603        ; $011640
        dc.w    $2000        ; $011642
        dc.w    $2038        ; $011644
        dc.w    $A032        ; $011646
        dc.w    $D1C0        ; $011648
        dc.w    $227C        ; $01164A
        dc.w    $2401        ; $01164C
        dc.w    $00A0        ; $01164E
        dc.w    $303C        ; $011650
        dc.w    $0080        ; $011652
        dc.w    $323C        ; $011654
        dc.w    $0050        ; $011656
        dc.w    $4EBA        ; $011658
        dc.w    $CD00        ; $01165A
        dc.w    $4AB8        ; $01165C
        dc.w    $A026        ; $01165E
        dc.w    $6700        ; $011660
        dc.w    $001C        ; $011662
        dc.w    $2038        ; $011664
        dc.w    $A022        ; $011666
        dc.w    $2238        ; $011668
        dc.w    $A026        ; $01166A
        dc.w    $D081        ; $01166C
        dc.w    $21C0        ; $01166E
        dc.w    $A022        ; $011670
        dc.w    $5338        ; $011672
        dc.w    $A02E        ; $011674
        dc.w    $6400        ; $011676
        dc.w    $009C        ; $011678
        dc.w    $42B8        ; $01167A
        dc.w    $A026        ; $01167C
        dc.w    $0C78        ; $01167E
        dc.w    $0001        ; $011680
        dc.w    $A05C        ; $011682
        dc.w    $6700        ; $011684
        dc.w    $0130        ; $011686
        dc.w    $0C78        ; $011688
        dc.w    $0002        ; $01168A
        dc.w    $A05C        ; $01168C
        dc.w    $6700        ; $01168E
        dc.w    $013C        ; $011690
        dc.w    $4EB9        ; $011692
        dc.w    $0088        ; $011694
        dc.w    $179E        ; $011696
        dc.w    $3238        ; $011698
        dc.w    $C86C        ; $01169A
        dc.w    $3401        ; $01169C
        dc.w    $0202        ; $01169E
        dc.w    $00F0        ; $0116A0
        dc.w    $672E        ; $0116A2
        dc.w    $11FC        ; $0116A4
        dc.w    $00A8        ; $0116A6
        dc.w    $C8A4        ; $0116A8
        dc.w    $31FC        ; $0116AA
        dc.w    $0002        ; $0116AC
        dc.w    $A05C        ; $0116AE
        dc.w    $11FC        ; $0116B0
        dc.w    $0001        ; $0116B2
        dc.w    $C809        ; $0116B4
        dc.w    $11FC        ; $0116B6
        dc.w    $0001        ; $0116B8
        dc.w    $C80A        ; $0116BA
        dc.w    $08F8        ; $0116BC
        dc.w    $0007        ; $0116BE
        dc.w    $C80E        ; $0116C0
        dc.w    $11FC        ; $0116C2
        dc.w    $0001        ; $0116C4
        dc.w    $C802        ; $0116C6
        dc.w    $4EB9        ; $0116C8
        dc.w    $0088        ; $0116CA
        dc.w    $205E        ; $0116CC
        dc.w    $6000        ; $0116CE
        dc.w    $0116        ; $0116D0
        dc.w    $E049        ; $0116D2
        dc.w    $0801        ; $0116D4
        dc.w    $0000        ; $0116D6
        dc.w    $671A        ; $0116D8
        dc.w    $4AB8        ; $0116DA
        dc.w    $A022        ; $0116DC
        dc.w    $6F00        ; $0116DE
        dc.w    $0034        ; $0116E0
        dc.w    $21FC        ; $0116E2
        dc.w    $FFFF        ; $0116E4
        dc.w    $FE00        ; $0116E6
        dc.w    $A026        ; $0116E8
        dc.w    $11FC        ; $0116EA
        dc.w    $0003        ; $0116EC
        dc.w    $A02E        ; $0116EE
        dc.w    $6000        ; $0116F0
        dc.w    $0022        ; $0116F2
        dc.w    $0801        ; $0116F4
        dc.w    $0001        ; $0116F6
        dc.w    $671A        ; $0116F8
        dc.w    $2038        ; $0116FA
        dc.w    $A022        ; $0116FC
        dc.w    $B0B8        ; $0116FE
        dc.w    $A02A        ; $011700
        dc.w    $6C00        ; $011702
        dc.w    $0010        ; $011704
        dc.w    $21FC        ; $011706
        dc.w    $0000        ; $011708
        dc.w    $0200        ; $01170A
        dc.w    $A026        ; $01170C
        dc.w    $11FC        ; $01170E
        dc.w    $0003        ; $011710
        dc.w    $A02E        ; $011712
        dc.w    $4AB8        ; $011714
        dc.w    $A036        ; $011716
        dc.w    $6700        ; $011718
        dc.w    $001C        ; $01171A
        dc.w    $2038        ; $01171C
        dc.w    $A032        ; $01171E
        dc.w    $2238        ; $011720
        dc.w    $A036        ; $011722
        dc.w    $D081        ; $011724
        dc.w    $21C0        ; $011726
        dc.w    $A032        ; $011728
        dc.w    $5338        ; $01172A
        dc.w    $A03E        ; $01172C
        dc.w    $6400        ; $01172E
        dc.w    $00B6        ; $011730
        dc.w    $42B8        ; $011732
        dc.w    $A036        ; $011734
        dc.w    $3238        ; $011736
        dc.w    $C86E        ; $011738
        dc.w    $3401        ; $01173A
        dc.w    $0202        ; $01173C
        dc.w    $00F0        ; $01173E
        dc.w    $672E        ; $011740
        dc.w    $11FC        ; $011742
        dc.w    $00A8        ; $011744
        dc.w    $C8A4        ; $011746
        dc.w    $31FC        ; $011748
        dc.w    $0002        ; $01174A
        dc.w    $A05C        ; $01174C
        dc.w    $11FC        ; $01174E
        dc.w    $0001        ; $011750
        dc.w    $C809        ; $011752
        dc.w    $11FC        ; $011754
        dc.w    $0001        ; $011756
        dc.w    $C80A        ; $011758
        dc.w    $08F8        ; $01175A
        dc.w    $0007        ; $01175C
        dc.w    $C80E        ; $01175E
        dc.w    $11FC        ; $011760
        dc.w    $0001        ; $011762
        dc.w    $C802        ; $011764
        dc.w    $4EB9        ; $011766
        dc.w    $0088        ; $011768
        dc.w    $205E        ; $01176A
        dc.w    $6000        ; $01176C
        dc.w    $0078        ; $01176E
        dc.w    $E049        ; $011770
        dc.w    $0801        ; $011772
        dc.w    $0000        ; $011774
        dc.w    $671A        ; $011776
        dc.w    $4AB8        ; $011778
        dc.w    $A032        ; $01177A
        dc.w    $6F00        ; $01177C
        dc.w    $0068        ; $01177E
        dc.w    $21FC        ; $011780
        dc.w    $FFFF        ; $011782
        dc.w    $FE00        ; $011784
        dc.w    $A036        ; $011786
        dc.w    $11FC        ; $011788
        dc.w    $0003        ; $01178A
        dc.w    $A03E        ; $01178C
        dc.w    $6000        ; $01178E
        dc.w    $0056        ; $011790
        dc.w    $0801        ; $011792
        dc.w    $0001        ; $011794
        dc.w    $674E        ; $011796
        dc.w    $2038        ; $011798
        dc.w    $A032        ; $01179A
        dc.w    $B0B8        ; $01179C
        dc.w    $A03A        ; $01179E
        dc.w    $6C00        ; $0117A0
        dc.w    $0044        ; $0117A2
        dc.w    $21FC        ; $0117A4
        dc.w    $0000        ; $0117A6
        dc.w    $0200        ; $0117A8
        dc.w    $A036        ; $0117AA
        dc.w    $11FC        ; $0117AC
        dc.w    $0003        ; $0117AE
        dc.w    $A03E        ; $0117B0
        dc.w    $6000        ; $0117B2
        dc.w    $0032        ; $0117B4
        dc.w    $4EB9        ; $0117B6
        dc.w    $0088        ; $0117B8
        dc.w    $FB36        ; $0117BA
        dc.w    $0838        ; $0117BC
        dc.w    $0006        ; $0117BE
        dc.w    $C80E        ; $0117C0
        dc.w    $6622        ; $0117C2
        dc.w    $4278        ; $0117C4
        dc.w    $A05C        ; $0117C6
        dc.w    $6000        ; $0117C8
        dc.w    $001C        ; $0117CA
        dc.w    $4EB9        ; $0117CC
        dc.w    $0088        ; $0117CE
        dc.w    $FB36        ; $0117D0
        dc.w    $0838        ; $0117D2
        dc.w    $0007        ; $0117D4
        dc.w    $C80E        ; $0117D6
        dc.w    $660C        ; $0117D8
        dc.w    $4278        ; $0117DA
        dc.w    $A05C        ; $0117DC
        dc.w    $5878        ; $0117DE
        dc.w    $C87E        ; $0117E0
        dc.w    $6000        ; $0117E2
        dc.w    $0006        ; $0117E4
        dc.w    $5978        ; $0117E6
        dc.w    $C87E        ; $0117E8
        dc.w    $33FC        ; $0117EA
        dc.w    $0018        ; $0117EC
        dc.w    $00FF        ; $0117EE
        dc.w    $0008        ; $0117F0
        dc.w    $4E75        ; $0117F2
        dc.w    $4A39        ; $0117F4
        dc.w    $00A1        ; $0117F6
        dc.w    $5120        ; $0117F8
        dc.w    $66F8        ; $0117FA
        dc.w    $4239        ; $0117FC
        dc.w    $00A1        ; $0117FE
        dc.w    $5123        ; $011800
        dc.w    $31FC        ; $011802
        dc.w    $0000        ; $011804
        dc.w    $C87E        ; $011806
        dc.w    $33FC        ; $011808
        dc.w    $0020        ; $01180A
        dc.w    $00FF        ; $01180C
        dc.w    $0008        ; $01180E
        dc.w    $4A38        ; $011810
        dc.w    $A019        ; $011812
        dc.w    $661A        ; $011814
        dc.w    $4A78        ; $011816
        dc.w    $A042        ; $011818
        dc.w    $6614        ; $01181A
        dc.w    $08F8        ; $01181C
        dc.w    $0003        ; $01181E
        dc.w    $C80E        ; $011820
        dc.w    $23FC        ; $011822
        dc.w    $0088        ; $011824
        dc.w    $4A3E        ; $011826
        dc.w    $00FF        ; $011828
        dc.w    $0002        ; $01182A
        dc.w    $6000        ; $01182C
        dc.w    $0032        ; $01182E
        dc.w    $08B8        ; $011830
        dc.w    $0003        ; $011832
        dc.w    $C80E        ; $011834
        dc.w    $08B8        ; $011836
        dc.w    $0007        ; $011838
        dc.w    $C81C        ; $01183A
        dc.w    $0838        ; $01183C
        dc.w    $0007        ; $01183E
        dc.w    $FEB7        ; $011840
        dc.w    $6712        ; $011842
        dc.w    $08F8        ; $011844
        dc.w    $0007        ; $011846
        dc.w    $C81C        ; $011848
        dc.w    $23FC        ; $01184A
        dc.w    $0088        ; $01184C
        dc.w    $C0F0        ; $01184E
        dc.w    $00FF        ; $011850
        dc.w    $0002        ; $011852
        dc.w    $600A        ; $011854
        dc.w    $23FC        ; $011856
        dc.w    $0088        ; $011858
        dc.w    $D48A        ; $01185A
        dc.w    $00FF        ; $01185C
        dc.w    $0002        ; $01185E
        dc.w    $4E75        ; $011860
        dc.w    $4A39        ; $011862
        dc.w    $00A1        ; $011864
        dc.w    $5120        ; $011866
        dc.w    $66F8        ; $011868
        dc.w    $4239        ; $01186A
        dc.w    $00A1        ; $01186C
        dc.w    $5123        ; $01186E
        dc.w    $31FC        ; $011870
        dc.w    $0000        ; $011872
        dc.w    $C87E        ; $011874
        dc.w    $33FC        ; $011876
        dc.w    $0020        ; $011878
        dc.w    $00FF        ; $01187A
        dc.w    $0008        ; $01187C
        dc.w    $23FC        ; $01187E
        dc.w    $0088        ; $011880
        dc.w    $D4A4        ; $011882
        dc.w    $00FF        ; $011884
        dc.w    $0002        ; $011886
        dc.w    $4E75        ; $011888
        dc.w    $41F9        ; $01188A
        dc.w    $00FF        ; $01188C
        dc.w    $6E00        ; $01188E
        dc.w    $303C        ; $011890
        dc.w    $007F        ; $011892
        dc.w    $4298        ; $011894
        dc.w    $51C8        ; $011896
        dc.w    $FFFC        ; $011898
        dc.w    $11FC        ; $01189A
        dc.w    $0001        ; $01189C
        dc.w    $C821        ; $01189E
        dc.w    $4EBA        ; $0118A0
        dc.w    $9E38        ; $0118A2
        dc.w    $0838        ; $0118A4
        dc.w    $0007        ; $0118A6
        dc.w    $C80E        ; $0118A8
        dc.w    $6626        ; $0118AA
        dc.w    $4A39        ; $0118AC
        dc.w    $00A1        ; $0118AE
        dc.w    $5120        ; $0118B0
        dc.w    $66F8        ; $0118B2
        dc.w    $4239        ; $0118B4
        dc.w    $00A1        ; $0118B6
        dc.w    $5123        ; $0118B8
        dc.w    $31FC        ; $0118BA
        dc.w    $0000        ; $0118BC
        dc.w    $C87E        ; $0118BE
        dc.w    $33FC        ; $0118C0
        dc.w    $0020        ; $0118C2
        dc.w    $00FF        ; $0118C4
        dc.w    $0008        ; $0118C6
        dc.w    $23FC        ; $0118C8
        dc.w    $0088        ; $0118CA
        dc.w    $D4B8        ; $0118CC
        dc.w    $00FF        ; $0118CE
        dc.w    $0002        ; $0118D0
        dc.w    $4E75        ; $0118D2
        dc.w    $161A        ; $0118D4
        dc.w    $6100        ; $0118D6
        dc.w    $0030        ; $0118D8
        dc.w    $323C        ; $0118DA
        dc.w    $000A        ; $0118DC
        dc.w    $6100        ; $0118DE
        dc.w    $0044        ; $0118E0
        dc.w    $5089        ; $0118E2
        dc.w    $161A        ; $0118E4
        dc.w    $6100        ; $0118E6
        dc.w    $0020        ; $0118E8
        dc.w    $323C        ; $0118EA
        dc.w    $000B        ; $0118EC
        dc.w    $6100        ; $0118EE
        dc.w    $0034        ; $0118F0
        dc.w    $5089        ; $0118F2
        dc.w    $121A        ; $0118F4
        dc.w    $0241        ; $0118F6
        dc.w    $000F        ; $0118F8
        dc.w    $6100        ; $0118FA
        dc.w    $0028        ; $0118FC
        dc.w    $5089        ; $0118FE
        dc.w    $161A        ; $011900
        dc.w    $6100        ; $011902
        dc.w    $0004        ; $011904
        dc.w    $4E75        ; $011906
        dc.w    $1203        ; $011908
        dc.w    $E809        ; $01190A
        dc.w    $0241        ; $01190C
        dc.w    $000F        ; $01190E
        dc.w    $6100        ; $011910
        dc.w    $0012        ; $011912
        dc.w    $5089        ; $011914
        dc.w    $3203        ; $011916
        dc.w    $0241        ; $011918
        dc.w    $000F        ; $01191A
        dc.w    $6100        ; $01191C
        dc.w    $0006        ; $01191E
        dc.w    $5089        ; $011920
        dc.w    $4E75        ; $011922
        dc.w    $ED49        ; $011924
        dc.w    $3001        ; $011926
        dc.w    $E349        ; $011928
        dc.w    $D240        ; $01192A
        dc.w    $207C        ; $01192C
        dc.w    $0601        ; $01192E
        dc.w    $DF00        ; $011930
        dc.w    $D0C1        ; $011932
        dc.w    $303C        ; $011934
        dc.w    $000C        ; $011936
        dc.w    $323C        ; $011938
        dc.w    $0010        ; $01193A
        dc.w    $4EBA        ; $01193C
        dc.w    $CA1C        ; $01193E
        dc.w    $4E75        ; $011940
        dc.w    $48E7        ; $011942
        dc.w    $1800        ; $011944
        dc.w    $161A        ; $011946
        dc.w    $6100        ; $011948
        dc.w    $0034        ; $01194A
        dc.w    $323C        ; $01194C
        dc.w    $000A        ; $01194E
        dc.w    $6100        ; $011950
        dc.w    $0048        ; $011952
        dc.w    $5089        ; $011954
        dc.w    $161A        ; $011956
        dc.w    $6100        ; $011958
        dc.w    $0024        ; $01195A
        dc.w    $323C        ; $01195C
        dc.w    $000B        ; $01195E
        dc.w    $6100        ; $011960
        dc.w    $0038        ; $011962
        dc.w    $5089        ; $011964
        dc.w    $121A        ; $011966
        dc.w    $0241        ; $011968
        dc.w    $000F        ; $01196A
        dc.w    $6100        ; $01196C
        dc.w    $002C        ; $01196E
        dc.w    $5089        ; $011970
        dc.w    $161A        ; $011972
        dc.w    $6100        ; $011974
        dc.w    $0008        ; $011976
        dc.w    $4CDF        ; $011978
        dc.w    $0018        ; $01197A
        dc.w    $4E75        ; $01197C
        dc.w    $1203        ; $01197E
        dc.w    $E809        ; $011980
        dc.w    $0241        ; $011982
        dc.w    $000F        ; $011984
        dc.w    $6100        ; $011986
        dc.w    $0012        ; $011988
        dc.w    $5089        ; $01198A
        dc.w    $3203        ; $01198C
        dc.w    $0241        ; $01198E
        dc.w    $000F        ; $011990
        dc.w    $6100        ; $011992
        dc.w    $0006        ; $011994
        dc.w    $5089        ; $011996
        dc.w    $4E75        ; $011998
        dc.w    $ED49        ; $01199A
        dc.w    $3001        ; $01199C
        dc.w    $E349        ; $01199E
        dc.w    $D240        ; $0119A0
        dc.w    $207C        ; $0119A2
        dc.w    $0601        ; $0119A4
        dc.w    $DF00        ; $0119A6
        dc.w    $D0C1        ; $0119A8
        dc.w    $303C        ; $0119AA
        dc.w    $000C        ; $0119AC
        dc.w    $323C        ; $0119AE
        dc.w    $0010        ; $0119B0
        dc.w    $4EBA        ; $0119B2
        dc.w    $00E4        ; $0119B4
        dc.w    $4E75        ; $0119B6
        dc.w    $41F9        ; $0119B8
        dc.w    $00FF        ; $0119BA
        dc.w    $6E00        ; $0119BC
        dc.w    $D1FC        ; $0119BE
        dc.w    $0000        ; $0119C0
        dc.w    $01E0        ; $0119C2
        dc.w    $43F9        ; $0119C4
        dc.w    $0089        ; $0119C6
        dc.w    $1A70        ; $0119C8
        dc.w    $303C        ; $0119CA
        dc.w    $0007        ; $0119CC
        dc.w    $3219        ; $0119CE
        dc.w    $08C1        ; $0119D0
        dc.w    $000F        ; $0119D2
        dc.w    $30C1        ; $0119D4
        dc.w    $51C8        ; $0119D6
        dc.w    $FFF6        ; $0119D8
        dc.w    $7000        ; $0119DA
        dc.w    $1038        ; $0119DC
        dc.w    $A019        ; $0119DE
        dc.w    $D080        ; $0119E0
        dc.w    $D080        ; $0119E2
        dc.w    $D080        ; $0119E4
        dc.w    $41F9        ; $0119E6
        dc.w    $00FF        ; $0119E8
        dc.w    $6E00        ; $0119EA
        dc.w    $D1FC        ; $0119EC
        dc.w    $0000        ; $0119EE
        dc.w    $01E0        ; $0119F0
        dc.w    $D1C0        ; $0119F2
        dc.w    $43F9        ; $0119F4
        dc.w    $0089        ; $0119F6
        dc.w    $1A80        ; $0119F8
        dc.w    $7200        ; $0119FA
        dc.w    $1238        ; $0119FC
        dc.w    $A01A        ; $0119FE
        dc.w    $343C        ; $011A00
        dc.w    $0003        ; $011A02
        dc.w    $3A19        ; $011A04
        dc.w    $6154        ; $011A06
        dc.w    $3605        ; $011A08
        dc.w    $3A19        ; $011A0A
        dc.w    $614E        ; $011A0C
        dc.w    $3805        ; $011A0E
        dc.w    $3A19        ; $011A10
        dc.w    $6148        ; $011A12
        dc.w    $EB4C        ; $011A14
        dc.w    $E14D        ; $011A16
        dc.w    $E54D        ; $011A18
        dc.w    $8644        ; $011A1A
        dc.w    $8645        ; $011A1C
        dc.w    $08C3        ; $011A1E
        dc.w    $000F        ; $011A20
        dc.w    $30C3        ; $011A22
        dc.w    $51CA        ; $011A24
        dc.w    $FFDE        ; $011A26
        dc.w    $1038        ; $011A28
        dc.w    $A01A        ; $011A2A
        dc.w    $1238        ; $011A2C
        dc.w    $A01C        ; $011A2E
        dc.w    $D001        ; $011A30
        dc.w    $0C00        ; $011A32
        dc.w    $001F        ; $011A34
        dc.w    $6F0A        ; $011A36
        dc.w    $103C        ; $011A38
        dc.w    $001F        ; $011A3A
        dc.w    $123C        ; $011A3C
        dc.w    $00FE        ; $011A3E
        dc.w    $600A        ; $011A40
        dc.w    $4A00        ; $011A42
        dc.w    $6C06        ; $011A44
        dc.w    $4200        ; $011A46
        dc.w    $123C        ; $011A48
        dc.w    $0002        ; $011A4A
        dc.w    $11C0        ; $011A4C
        dc.w    $A01A        ; $011A4E
        dc.w    $11C1        ; $011A50
        dc.w    $A01C        ; $011A52
        dc.w    $11FC        ; $011A54
        dc.w    $0001        ; $011A56
        dc.w    $C821        ; $011A58
        dc.w    $4E75        ; $011A5A
        dc.w    $DA41        ; $011A5C
        dc.w    $0C45        ; $011A5E
        dc.w    $001F        ; $011A60
        dc.w    $6F04        ; $011A62
        dc.w    $3A3C        ; $011A64
        dc.w    $001F        ; $011A66
        dc.w    $4A45        ; $011A68
        dc.w    $6E02        ; $011A6A
        dc.w    $4245        ; $011A6C
        dc.w    $4E75        ; $011A6E
        dc.w    $4400        ; $011A70
        dc.w    $44A3        ; $011A72
        dc.w    $4946        ; $011A74
        dc.w    $4DE9        ; $011A76
        dc.w    $4400        ; $011A78
        dc.w    $44A3        ; $011A7A
        dc.w    $4946        ; $011A7C
        dc.w    $4DE9        ; $011A7E
        dc.w    $0000        ; $011A80
        dc.w    $0000        ; $011A82
        dc.w    $0011        ; $011A84
        dc.w    $0003        ; $011A86
        dc.w    $0005        ; $011A88
        dc.w    $0011        ; $011A8A
        dc.w    $0006        ; $011A8C
        dc.w    $000A        ; $011A8E
        dc.w    $0012        ; $011A90
        dc.w    $0008        ; $011A92
        dc.w    $000F        ; $011A94
        dc.w    $0013        ; $011A96
        dc.w    $4A39        ; $011A98
        dc.w    $00A1        ; $011A9A
        dc.w    $5120        ; $011A9C
        dc.w    $66F8        ; $011A9E
        dc.w    $23C9        ; $011AA0
        dc.w    $00A1        ; $011AA2
        dc.w    $5128        ; $011AA4
        dc.w    $13FC        ; $011AA6
        dc.w    $0001        ; $011AA8
        dc.w    $00A1        ; $011AAA
        dc.w    $512C        ; $011AAC
        dc.w    $13FC        ; $011AAE
        dc.w    $0020        ; $011AB0
        dc.w    $00A1        ; $011AB2
        dc.w    $5121        ; $011AB4
        dc.w    $13FC        ; $011AB6
        dc.w    $0001        ; $011AB8
        dc.w    $00A1        ; $011ABA
        dc.w    $5120        ; $011ABC
        dc.w    $4A39        ; $011ABE
        dc.w    $00A1        ; $011AC0
        dc.w    $512C        ; $011AC2
        dc.w    $66F8        ; $011AC4
        dc.w    $33C0        ; $011AC6
        dc.w    $00A1        ; $011AC8
        dc.w    $5128        ; $011ACA
        dc.w    $33C1        ; $011ACC
        dc.w    $00A1        ; $011ACE
        dc.w    $512A        ; $011AD0
        dc.w    $13FC        ; $011AD2
        dc.w    $0001        ; $011AD4
        dc.w    $00A1        ; $011AD6
        dc.w    $512C        ; $011AD8
        dc.w    $4A39        ; $011ADA
        dc.w    $00A1        ; $011ADC
        dc.w    $512C        ; $011ADE
        dc.w    $66F8        ; $011AE0
        dc.w    $33C2        ; $011AE2
        dc.w    $00A1        ; $011AE4
        dc.w    $5128        ; $011AE6
        dc.w    $13FC        ; $011AE8
        dc.w    $0001        ; $011AEA
        dc.w    $00A1        ; $011AEC
        dc.w    $512C        ; $011AEE
        dc.w    $4A39        ; $011AF0
        dc.w    $00A1        ; $011AF2
        dc.w    $512C        ; $011AF4
        dc.w    $66F8        ; $011AF6
        dc.w    $23C8        ; $011AF8
        dc.w    $00A1        ; $011AFA
        dc.w    $5128        ; $011AFC
        dc.w    $13FC        ; $011AFE
        dc.w    $0001        ; $011B00
        dc.w    $00A1        ; $011B02
        dc.w    $512C        ; $011B04
        dc.w    $4E75        ; $011B06
        dc.w    $207C        ; $011B08
        dc.w    $0601        ; $011B0A
        dc.w    $8000        ; $011B0C
        dc.w    $227C        ; $011B0E
        dc.w    $0400        ; $011B10
        dc.w    $4C78        ; $011B12
        dc.w    $303C        ; $011B14
        dc.w    $0050        ; $011B16
        dc.w    $323C        ; $011B18
        dc.w    $0010        ; $011B1A
        dc.w    $4EBA        ; $011B1C
        dc.w    $C83C        ; $011B1E
        dc.w    $207C        ; $011B20
        dc.w    $0601        ; $011B22
        dc.w    $E8C0        ; $011B24
        dc.w    $227C        ; $011B26
        dc.w    $0400        ; $011B28
        dc.w    $8090        ; $011B2A
        dc.w    $303C        ; $011B2C
        dc.w    $0058        ; $011B2E
        dc.w    $323C        ; $011B30
        dc.w    $0010        ; $011B32
        dc.w    $4EBA        ; $011B34
        dc.w    $C824        ; $011B36
        dc.w    $207C        ; $011B38
        dc.w    $0601        ; $011B3A
        dc.w    $EE40        ; $011B3C
        dc.w    $227C        ; $011B3E
        dc.w    $0400        ; $011B40
        dc.w    $A090        ; $011B42
        dc.w    $303C        ; $011B44
        dc.w    $0058        ; $011B46
        dc.w    $323C        ; $011B48
        dc.w    $0010        ; $011B4A
        dc.w    $4EBA        ; $011B4C
        dc.w    $C80C        ; $011B4E
        dc.w    $207C        ; $011B50
        dc.w    $0601        ; $011B52
        dc.w    $8500        ; $011B54
        dc.w    $227C        ; $011B56
        dc.w    $0400        ; $011B58
        dc.w    $8048        ; $011B5A
        dc.w    $303C        ; $011B5C
        dc.w    $0038        ; $011B5E
        dc.w    $323C        ; $011B60
        dc.w    $0020        ; $011B62
        dc.w    $4EBA        ; $011B64
        dc.w    $C7F4        ; $011B66
        dc.w    $4E75        ; $011B68
        dc.w    $2038        ; $011B6A
        dc.w    $C260        ; $011B6C
        dc.w    $21C0        ; $011B6E
        dc.w    $A058        ; $011B70
        dc.w    $43F8        ; $011B72
        dc.w    $B400        ; $011B74
        dc.w    $45F8        ; $011B76
        dc.w    $C400        ; $011B78
        dc.w    $7E1F        ; $011B7A
        dc.w    $4CD9        ; $011B7C
        dc.w    $087F        ; $011B7E
        dc.w    $48E2        ; $011B80
        dc.w    $FE10        ; $011B82
        dc.w    $51CF        ; $011B84
        dc.w    $FFF6        ; $011B86
        dc.w    $42B8        ; $011B88
        dc.w    $A04A        ; $011B8A
        dc.w    $41F8        ; $011B8C
        dc.w    $A04A        ; $011B8E
        dc.w    $43F8        ; $011B90
        dc.w    $C200        ; $011B92
        dc.w    $343C        ; $011B94
        dc.w    $0013        ; $011B96
        dc.w    $0600        ; $011B98
        dc.w    $0000        ; $011B9A
        dc.w    $1028        ; $011B9C
        dc.w    $0003        ; $011B9E
        dc.w    $1229        ; $011BA0
        dc.w    $0003        ; $011BA2
        dc.w    $C101        ; $011BA4
        dc.w    $1140        ; $011BA6
        dc.w    $0003        ; $011BA8
        dc.w    $1028        ; $011BAA
        dc.w    $0002        ; $011BAC
        dc.w    $1229        ; $011BAE
        dc.w    $0002        ; $011BB0
        dc.w    $C101        ; $011BB2
        dc.w    $1200        ; $011BB4
        dc.w    $0200        ; $011BB6
        dc.w    $000F        ; $011BB8
        dc.w    $1140        ; $011BBA
        dc.w    $0002        ; $011BBC
        dc.w    $E809        ; $011BBE
        dc.w    $0600        ; $011BC0
        dc.w    $0000        ; $011BC2
        dc.w    $1028        ; $011BC4
        dc.w    $0001        ; $011BC6
        dc.w    $C101        ; $011BC8
        dc.w    $1229        ; $011BCA
        dc.w    $0001        ; $011BCC
        dc.w    $C101        ; $011BCE
        dc.w    $6400        ; $011BD0
        dc.w    $0012        ; $011BD2
        dc.w    $0600        ; $011BD4
        dc.w    $0000        ; $011BD6
        dc.w    $123C        ; $011BD8
        dc.w    $0040        ; $011BDA
        dc.w    $C101        ; $011BDC
        dc.w    $123C        ; $011BDE
        dc.w    $0001        ; $011BE0
        dc.w    $6018        ; $011BE2
        dc.w    $4201        ; $011BE4
        dc.w    $0C00        ; $011BE6
        dc.w    $0060        ; $011BE8
        dc.w    $6500        ; $011BEA
        dc.w    $0010        ; $011BEC
        dc.w    $0600        ; $011BEE
        dc.w    $0000        ; $011BF0
        dc.w    $123C        ; $011BF2
        dc.w    $0060        ; $011BF4
        dc.w    $8101        ; $011BF6
        dc.w    $123C        ; $011BF8
        dc.w    $0001        ; $011BFA
        dc.w    $1140        ; $011BFC
        dc.w    $0001        ; $011BFE
        dc.w    $0600        ; $011C00
        dc.w    $0000        ; $011C02
        dc.w    $1010        ; $011C04
        dc.w    $C101        ; $011C06
        dc.w    $1211        ; $011C08
        dc.w    $C101        ; $011C0A
        dc.w    $1080        ; $011C0C
        dc.w    $5889        ; $011C0E
        dc.w    $51CA        ; $011C10
        dc.w    $FF86        ; $011C12
        dc.w    $4AB8        ; $011C14
        dc.w    $A04A        ; $011C16
        dc.w    $6608        ; $011C18
        dc.w    $21FC        ; $011C1A
        dc.w    $CCCC        ; $011C1C
        dc.w    $0CCC        ; $011C1E
        dc.w    $A04A        ; $011C20
        dc.w    $41F8        ; $011C22
        dc.w    $C200        ; $011C24
        dc.w    $303C        ; $011C26
        dc.w    $0013        ; $011C28
        dc.w    $4A90        ; $011C2A
        dc.w    $6700        ; $011C2C
        dc.w    $000A        ; $011C2E
        dc.w    $5888        ; $011C30
        dc.w    $51C8        ; $011C32
        dc.w    $FFF6        ; $011C34
        dc.w    $600A        ; $011C36
        dc.w    $20FC        ; $011C38
        dc.w    $CCCC        ; $011C3A
        dc.w    $0CCC        ; $011C3C
        dc.w    $51C8        ; $011C3E
        dc.w    $FFF8        ; $011C40
        dc.w    $31FC        ; $011C42
        dc.w    $0002        ; $011C44
        dc.w    $A04E        ; $011C46
        dc.w    $2038        ; $011C48
        dc.w    $A058        ; $011C4A
        dc.w    $2238        ; $011C4C
        dc.w    $C260        ; $011C4E
        dc.w    $B280        ; $011C50
        dc.w    $6700        ; $011C52
        dc.w    $0028        ; $011C54
        dc.w    $31FC        ; $011C56
        dc.w    $0001        ; $011C58
        dc.w    $A04E        ; $011C5A
        dc.w    $0C80        ; $011C5C
        dc.w    $CCCC        ; $011C5E
        dc.w    $0CCC        ; $011C60
        dc.w    $6718        ; $011C62
        dc.w    $4278        ; $011C64
        dc.w    $A04E        ; $011C66
        dc.w    $0C81        ; $011C68
        dc.w    $CCCC        ; $011C6A
        dc.w    $0CCC        ; $011C6C
        dc.w    $670C        ; $011C6E
        dc.w    $B280        ; $011C70
        dc.w    $6200        ; $011C72
        dc.w    $0008        ; $011C74
        dc.w    $31FC        ; $011C76
        dc.w    $0001        ; $011C78
        dc.w    $A04E        ; $011C7A
        dc.w    $4E75        ; $011C7C
        dc.w    $4A78        ; $011C7E
        dc.w    $A04E        ; $011C80
        dc.w    $6640        ; $011C82
        dc.w    $0838        ; $011C84
        dc.w    $0000        ; $011C86
        dc.w    $A050        ; $011C88
        dc.w    $661C        ; $011C8A
        dc.w    $207C        ; $011C8C
        dc.w    $0601        ; $011C8E
        dc.w    $8F80        ; $011C90
        dc.w    $227C        ; $011C92
        dc.w    $0400        ; $011C94
        dc.w    $D018        ; $011C96
        dc.w    $303C        ; $011C98
        dc.w    $0078        ; $011C9A
        dc.w    $323C        ; $011C9C
        dc.w    $0018        ; $011C9E
        dc.w    $4EBA        ; $011CA0
        dc.w    $C6B8        ; $011CA2
        dc.w    $6000        ; $011CA4
        dc.w    $0062        ; $011CA6
        dc.w    $207C        ; $011CA8
        dc.w    $0601        ; $011CAA
        dc.w    $0000        ; $011CAC
        dc.w    $227C        ; $011CAE
        dc.w    $0400        ; $011CB0
        dc.w    $D018        ; $011CB2
        dc.w    $303C        ; $011CB4
        dc.w    $0078        ; $011CB6
        dc.w    $323C        ; $011CB8
        dc.w    $0018        ; $011CBA
        dc.w    $4EBA        ; $011CBC
        dc.w    $C69C        ; $011CBE
        dc.w    $6000        ; $011CC0
        dc.w    $0046        ; $011CC2
        dc.w    $0C78        ; $011CC4
        dc.w    $0002        ; $011CC6
        dc.w    $A04E        ; $011CC8
        dc.w    $673C        ; $011CCA
        dc.w    $0838        ; $011CCC
        dc.w    $0000        ; $011CCE
        dc.w    $A050        ; $011CD0
        dc.w    $661C        ; $011CD2
        dc.w    $207C        ; $011CD4
        dc.w    $0601        ; $011CD6
        dc.w    $9AC0        ; $011CD8
        dc.w    $227C        ; $011CDA
        dc.w    $0400        ; $011CDC
        dc.w    $D0A0        ; $011CDE
        dc.w    $303C        ; $011CE0
        dc.w    $0078        ; $011CE2
        dc.w    $323C        ; $011CE4
        dc.w    $0018        ; $011CE6
        dc.w    $4EBA        ; $011CE8
        dc.w    $C670        ; $011CEA
        dc.w    $6000        ; $011CEC
        dc.w    $001A        ; $011CEE
        dc.w    $207C        ; $011CF0
        dc.w    $0601        ; $011CF2
        dc.w    $0000        ; $011CF4
        dc.w    $227C        ; $011CF6
        dc.w    $0400        ; $011CF8
        dc.w    $D0A0        ; $011CFA
        dc.w    $303C        ; $011CFC
        dc.w    $0078        ; $011CFE
        dc.w    $323C        ; $011D00
        dc.w    $0018        ; $011D02
        dc.w    $4EBA        ; $011D04
        dc.w    $C654        ; $011D06
        dc.w    $4E75        ; $011D08
        dc.w    $33FC        ; $011D0A
        dc.w    $002C        ; $011D0C
        dc.w    $00FF        ; $011D0E
        dc.w    $0008        ; $011D10
        dc.w    $31FC        ; $011D12
        dc.w    $002C        ; $011D14
        dc.w    $C87A        ; $011D16
        dc.w    $08B8        ; $011D18
        dc.w    $0006        ; $011D1A
        dc.w    $C875        ; $011D1C
        dc.w    $3AB8        ; $011D1E
        dc.w    $C874        ; $011D20
        dc.w    $33FC        ; $011D22
        dc.w    $0083        ; $011D24
        dc.w    $00A1        ; $011D26
        dc.w    $5100        ; $011D28
        dc.w    $0239        ; $011D2A
        dc.w    $00FC        ; $011D2C
        dc.w    $00A1        ; $011D2E
        dc.w    $5181        ; $011D30
        dc.w    $4EB9        ; $011D32
        dc.w    $0088        ; $011D34
        dc.w    $26C8        ; $011D36
        dc.w    $203C        ; $011D38
        dc.w    $000A        ; $011D3A
        dc.w    $0907        ; $011D3C
        dc.w    $4EB9        ; $011D3E
        dc.w    $0088        ; $011D40
        dc.w    $14BE        ; $011D42
        dc.w    $11FC        ; $011D44
        dc.w    $0001        ; $011D46
        dc.w    $C80D        ; $011D48
        dc.w    $7000        ; $011D4A
        dc.w    $41F8        ; $011D4C
        dc.w    $8480        ; $011D4E
        dc.w    $721F        ; $011D50
        dc.w    $20C0        ; $011D52
        dc.w    $51C9        ; $011D54
        dc.w    $FFFC        ; $011D56
        dc.w    $41F9        ; $011D58
        dc.w    $00FF        ; $011D5A
        dc.w    $7B80        ; $011D5C
        dc.w    $727F        ; $011D5E
        dc.w    $20C0        ; $011D60
        dc.w    $51C9        ; $011D62
        dc.w    $FFFC        ; $011D64
        dc.w    $2ABC        ; $011D66
        dc.w    $6000        ; $011D68
        dc.w    $0002        ; $011D6A
        dc.w    $323C        ; $011D6C
        dc.w    $17FF        ; $011D6E
        dc.w    $2C80        ; $011D70
        dc.w    $51C9        ; $011D72
        dc.w    $FFFC        ; $011D74
        dc.w    $4EB9        ; $011D76
        dc.w    $0088        ; $011D78
        dc.w    $49AA        ; $011D7A
        dc.w    $4278        ; $011D7C
        dc.w    $C880        ; $011D7E
        dc.w    $4278        ; $011D80
        dc.w    $C882        ; $011D82
        dc.w    $4278        ; $011D84
        dc.w    $8000        ; $011D86
        dc.w    $4278        ; $011D88
        dc.w    $8002        ; $011D8A
        dc.w    $4278        ; $011D8C
        dc.w    $A012        ; $011D8E
        dc.w    $4238        ; $011D90
        dc.w    $A018        ; $011D92
        dc.w    $4EB9        ; $011D94
        dc.w    $0088        ; $011D96
        dc.w    $49AA        ; $011D98
        dc.w    $21FC        ; $011D9A
        dc.w    $008B        ; $011D9C
        dc.w    $B4FC        ; $011D9E
        dc.w    $C96C        ; $011DA0
        dc.w    $11FC        ; $011DA2
        dc.w    $0001        ; $011DA4
        dc.w    $C809        ; $011DA6
        dc.w    $11FC        ; $011DA8
        dc.w    $0001        ; $011DAA
        dc.w    $C80A        ; $011DAC
        dc.w    $08F8        ; $011DAE
        dc.w    $0006        ; $011DB0
        dc.w    $C80E        ; $011DB2
        dc.w    $11FC        ; $011DB4
        dc.w    $0001        ; $011DB6
        dc.w    $C802        ; $011DB8
        dc.w    $31FC        ; $011DBA
        dc.w    $0001        ; $011DBC
        dc.w    $A038        ; $011DBE
        dc.w    $41F9        ; $011DC0
        dc.w    $00FF        ; $011DC2
        dc.w    $1000        ; $011DC4
        dc.w    $303C        ; $011DC6
        dc.w    $037F        ; $011DC8
        dc.w    $4298        ; $011DCA
        dc.w    $51C8        ; $011DCC
        dc.w    $FFFC        ; $011DCE
        dc.w    $303C        ; $011DD0
        dc.w    $0001        ; $011DD2
        dc.w    $323C        ; $011DD4
        dc.w    $0001        ; $011DD6
        dc.w    $343C        ; $011DD8
        dc.w    $0001        ; $011DDA
        dc.w    $363C        ; $011DDC
        dc.w    $0026        ; $011DDE
        dc.w    $383C        ; $011DE0
        dc.w    $001A        ; $011DE2
        dc.w    $41F9        ; $011DE4
        dc.w    $00FF        ; $011DE6
        dc.w    $1000        ; $011DE8
        dc.w    $4EBA        ; $011DEA
        dc.w    $C440        ; $011DEC
        dc.w    $41F9        ; $011DEE
        dc.w    $00FF        ; $011DF0
        dc.w    $1000        ; $011DF2
        dc.w    $4EBA        ; $011DF4
        dc.w    $C4FA        ; $011DF6
        dc.w    $4EBA        ; $011DF8
        dc.w    $C3C2        ; $011DFA
        dc.w    $08B9        ; $011DFC
        dc.w    $0007        ; $011DFE
        dc.w    $00A1        ; $011E00
        dc.w    $5181        ; $011E02
        dc.w    $41F9        ; $011E04
        dc.w    $00FF        ; $011E06
        dc.w    $6E00        ; $011E08
        dc.w    $5488        ; $011E0A
        dc.w    $43F9        ; $011E0C
        dc.w    $0089        ; $011E0E
        dc.w    $1F38        ; $011E10
        dc.w    $303C        ; $011E12
        dc.w    $002E        ; $011E14
        dc.w    $3219        ; $011E16
        dc.w    $08C1        ; $011E18
        dc.w    $000F        ; $011E1A
        dc.w    $30C1        ; $011E1C
        dc.w    $51C8        ; $011E1E
        dc.w    $FFF6        ; $011E20
        dc.w    $41F9        ; $011E22
        dc.w    $00FF        ; $011E24
        dc.w    $6E00        ; $011E26
        dc.w    $D1FC        ; $011E28
        dc.w    $0000        ; $011E2A
        dc.w    $0120        ; $011E2C
        dc.w    $43F9        ; $011E2E
        dc.w    $0089        ; $011E30
        dc.w    $1F96        ; $011E32
        dc.w    $303C        ; $011E34
        dc.w    $005F        ; $011E36
        dc.w    $3219        ; $011E38
        dc.w    $08C1        ; $011E3A
        dc.w    $000F        ; $011E3C
        dc.w    $30C1        ; $011E3E
        dc.w    $51C8        ; $011E40
        dc.w    $FFF6        ; $011E42
        dc.w    $41F9        ; $011E44
        dc.w    $000E        ; $011E46
        dc.w    $DF70        ; $011E48
        dc.w    $227C        ; $011E4A
        dc.w    $0601        ; $011E4C
        dc.w    $8000        ; $011E4E
        dc.w    $4EBA        ; $011E50
        dc.w    $C4C4        ; $011E52
        dc.w    $41F9        ; $011E54
        dc.w    $000E        ; $011E56
        dc.w    $E770        ; $011E58
        dc.w    $227C        ; $011E5A
        dc.w    $0601        ; $011E5C
        dc.w    $AD00        ; $011E5E
        dc.w    $4EBA        ; $011E60
        dc.w    $C4B4        ; $011E62
        dc.w    $41F9        ; $011E64
        dc.w    $000E        ; $011E66
        dc.w    $BB40        ; $011E68
        dc.w    $227C        ; $011E6A
        dc.w    $0601        ; $011E6C
        dc.w    $BE00        ; $011E6E
        dc.w    $4EBA        ; $011E70
        dc.w    $C4A4        ; $011E72
        dc.w    $41F9        ; $011E74
        dc.w    $000E        ; $011E76
        dc.w    $B980        ; $011E78
        dc.w    $227C        ; $011E7A
        dc.w    $0601        ; $011E7C
        dc.w    $F000        ; $011E7E
        dc.w    $4EBA        ; $011E80
        dc.w    $C494        ; $011E82
        dc.w    $41F9        ; $011E84
        dc.w    $000E        ; $011E86
        dc.w    $C6F0        ; $011E88
        dc.w    $227C        ; $011E8A
        dc.w    $0601        ; $011E8C
        dc.w    $F9C0        ; $011E8E
        dc.w    $4EBA        ; $011E90
        dc.w    $C484        ; $011E92
        dc.w    $41F9        ; $011E94
        dc.w    $000E        ; $011E96
        dc.w    $C150        ; $011E98
        dc.w    $227C        ; $011E9A
        dc.w    $0602        ; $011E9C
        dc.w    $07C0        ; $011E9E
        dc.w    $4EBA        ; $011EA0
        dc.w    $C474        ; $011EA2
        dc.w    $41F9        ; $011EA4
        dc.w    $000E        ; $011EA6
        dc.w    $EA40        ; $011EA8
        dc.w    $227C        ; $011EAA
        dc.w    $0603        ; $011EAC
        dc.w    $0000        ; $011EAE
        dc.w    $4EBA        ; $011EB0
        dc.w    $C464        ; $011EB2
        dc.w    $11FC        ; $011EB4
        dc.w    $0000        ; $011EB6
        dc.w    $A019        ; $011EB8
        dc.w    $11FC        ; $011EBA
        dc.w    $0000        ; $011EBC
        dc.w    $A01A        ; $011EBE
        dc.w    $11FC        ; $011EC0
        dc.w    $0000        ; $011EC2
        dc.w    $A01B        ; $011EC4
        dc.w    $11FC        ; $011EC6
        dc.w    $0000        ; $011EC8
        dc.w    $A01C        ; $011ECA
        dc.w    $42B8        ; $011ECC
        dc.w    $A026        ; $011ECE
        dc.w    $4278        ; $011ED0
        dc.w    $A02C        ; $011ED2
        dc.w    $21FC        ; $011ED4
        dc.w    $0402        ; $011ED6
        dc.w    $A060        ; $011ED8
        dc.w    $A030        ; $011EDA
        dc.w    $21FC        ; $011EDC
        dc.w    $0402        ; $011EDE
        dc.w    $A020        ; $011EE0
        dc.w    $A034        ; $011EE2
        dc.w    $4EB9        ; $011EE4
        dc.w    $0088        ; $011EE6
        dc.w    $204A        ; $011EE8
        dc.w    $11FC        ; $011EEA
        dc.w    $0001        ; $011EEC
        dc.w    $C821        ; $011EEE
        dc.w    $0239        ; $011EF0
        dc.w    $00FC        ; $011EF2
        dc.w    $00A1        ; $011EF4
        dc.w    $5181        ; $011EF6
        dc.w    $0039        ; $011EF8
        dc.w    $0001        ; $011EFA
        dc.w    $00A1        ; $011EFC
        dc.w    $5181        ; $011EFE
        dc.w    $33FC        ; $011F00
        dc.w    $8083        ; $011F02
        dc.w    $00A1        ; $011F04
        dc.w    $5100        ; $011F06
        dc.w    $08F8        ; $011F08
        dc.w    $0006        ; $011F0A
        dc.w    $C875        ; $011F0C
        dc.w    $3AB8        ; $011F0E
        dc.w    $C874        ; $011F10
        dc.w    $33FC        ; $011F12
        dc.w    $0018        ; $011F14
        dc.w    $00FF        ; $011F16
        dc.w    $0008        ; $011F18
        dc.w    $4EB9        ; $011F1A
        dc.w    $0088        ; $011F1C
        dc.w    $4998        ; $011F1E
        dc.w    $31FC        ; $011F20
        dc.w    $0000        ; $011F22
        dc.w    $C87E        ; $011F24
        dc.w    $23FC        ; $011F26
        dc.w    $0089        ; $011F28
        dc.w    $2056        ; $011F2A
        dc.w    $00FF        ; $011F2C
        dc.w    $0002        ; $011F2E
        dc.w    $11FC        ; $011F30
        dc.w    $008F        ; $011F32
        dc.w    $C8A5        ; $011F34
        dc.w    $4E75        ; $011F36
        dc.w    $0000        ; $011F38
        dc.w    $0421        ; $011F3A
        dc.w    $0842        ; $011F3C
        dc.w    $0C63        ; $011F3E
        dc.w    $1084        ; $011F40
        dc.w    $14A5        ; $011F42
        dc.w    $18C6        ; $011F44
        dc.w    $1CE7        ; $011F46
        dc.w    $2108        ; $011F48
        dc.w    $2529        ; $011F4A
        dc.w    $7FFF        ; $011F4C
        dc.w    $7FFF        ; $011F4E
        dc.w    $318C        ; $011F50
        dc.w    $7FFF        ; $011F52
        dc.w    $39CE        ; $011F54
        dc.w    $3DEF        ; $011F56
        dc.w    $7FFF        ; $011F58
        dc.w    $7FFF        ; $011F5A
        dc.w    $7FFF        ; $011F5C
        dc.w    $7FFF        ; $011F5E
        dc.w    $5294        ; $011F60
        dc.w    $56B5        ; $011F62
        dc.w    $7FFF        ; $011F64
        dc.w    $5EF7        ; $011F66
        dc.w    $6318        ; $011F68
        dc.w    $7FFF        ; $011F6A
        dc.w    $6B5A        ; $011F6C
        dc.w    $6F7B        ; $011F6E
        dc.w    $7FFF        ; $011F70
        dc.w    $7FFF        ; $011F72
        dc.w    $7BDE        ; $011F74
        dc.w    $7FFF        ; $011F76
        dc.w    $7FFF        ; $011F78
        dc.w    $7FFF        ; $011F7A
        dc.w    $7FFF        ; $011F7C
        dc.w    $7FFF        ; $011F7E
        dc.w    $7FFF        ; $011F80
        dc.w    $7FFF        ; $011F82
        dc.w    $7FFF        ; $011F84
        dc.w    $7FFF        ; $011F86
        dc.w    $7FFF        ; $011F88
        dc.w    $7FFF        ; $011F8A
        dc.w    $7FFF        ; $011F8C
        dc.w    $7FFF        ; $011F8E
        dc.w    $7FFF        ; $011F90
        dc.w    $7FFF        ; $011F92
        dc.w    $7FFF        ; $011F94
        dc.w    $6337        ; $011F96
        dc.w    $6737        ; $011F98
        dc.w    $6B58        ; $011F9A
        dc.w    $6F79        ; $011F9C
        dc.w    $4445        ; $011F9E
        dc.w    $512B        ; $011FA0
        dc.w    $6212        ; $011FA2
        dc.w    $6EF8        ; $011FA4
        dc.w    $739A        ; $011FA6
        dc.w    $61E8        ; $011FA8
        dc.w    $7FFF        ; $011FAA
        dc.w    $7FFF        ; $011FAC
        dc.w    $7FFF        ; $011FAE
        dc.w    $7FFF        ; $011FB0
        dc.w    $7FFF        ; $011FB2
        dc.w    $7FFF        ; $011FB4
        dc.w    $7FFF        ; $011FB6
        dc.w    $7000        ; $011FB8
        dc.w    $7CA0        ; $011FBA
        dc.w    $70E7        ; $011FBC
        dc.w    $7FFF        ; $011FBE
        dc.w    $5800        ; $011FC0
        dc.w    $0180        ; $011FC2
        dc.w    $0200        ; $011FC4
        dc.w    $000E        ; $011FC6
        dc.w    $0014        ; $011FC8
        dc.w    $1CFB        ; $011FCA
        dc.w    $7FFF        ; $011FCC
        dc.w    $7FFF        ; $011FCE
        dc.w    $7FFF        ; $011FD0
        dc.w    $7FFF        ; $011FD2
        dc.w    $7FFF        ; $011FD4
        dc.w    $4400        ; $011FD6
        dc.w    $44A3        ; $011FD8
        dc.w    $4946        ; $011FDA
        dc.w    $4DE9        ; $011FDC
        dc.w    $1C00        ; $011FDE
        dc.w    $28A3        ; $011FE0
        dc.w    $3546        ; $011FE2
        dc.w    $41E9        ; $011FE4
        dc.w    $5940        ; $011FE6
        dc.w    $5983        ; $011FE8
        dc.w    $55E6        ; $011FEA
        dc.w    $5629        ; $011FEC
        dc.w    $1C00        ; $011FEE
        dc.w    $28A3        ; $011FF0
        dc.w    $3546        ; $011FF2
        dc.w    $41E9        ; $011FF4
        dc.w    $4400        ; $011FF6
        dc.w    $44A3        ; $011FF8
        dc.w    $4946        ; $011FFA
        dc.w    $4DE9        ; $011FFC
        dc.w    $7FFF        ; $011FFE
        dc.w    $63F5        ; $012000
        dc.w    $7FFF        ; $012002
        dc.w    $7FFF        ; $012004
        dc.w    $0010        ; $012006
        dc.w    $7FFF        ; $012008
        dc.w    $294E        ; $01200A
        dc.w    $7FFF        ; $01200C
        dc.w    $0000        ; $01200E
        dc.w    $4E73        ; $012010
        dc.w    $6739        ; $012012
        dc.w    $7FFF        ; $012014
        dc.w    $6337        ; $012016
        dc.w    $6737        ; $012018
        dc.w    $6B58        ; $01201A
        dc.w    $6F79        ; $01201C
        dc.w    $6B36        ; $01201E
        dc.w    $6B37        ; $012020
        dc.w    $6F58        ; $012022
        dc.w    $6F79        ; $012024
        dc.w    $739A        ; $012026
        dc.w    $61E8        ; $012028
        dc.w    $7FFF        ; $01202A
        dc.w    $7FFF        ; $01202C
        dc.w    $7FFF        ; $01202E
        dc.w    $7FFF        ; $012030
        dc.w    $7FFF        ; $012032
        dc.w    $7FFF        ; $012034
        dc.w    $10E1        ; $012036
        dc.w    $29A8        ; $012038
        dc.w    $4670        ; $01203A
        dc.w    $6337        ; $01203C
        dc.w    $4445        ; $01203E
        dc.w    $512B        ; $012040
        dc.w    $6212        ; $012042
        dc.w    $6EF8        ; $012044
        dc.w    $7FFF        ; $012046
        dc.w    $031F        ; $012048
        dc.w    $7FFF        ; $01204A
        dc.w    $7FFF        ; $01204C
        dc.w    $7FFF        ; $01204E
        dc.w    $7FFF        ; $012050
        dc.w    $7FFF        ; $012052
        dc.w    $7FFF        ; $012054
        dc.w    $4EB9        ; $012056
        dc.w    $0088        ; $012058
        dc.w    $2080        ; $01205A
        dc.w    $3038        ; $01205C
        dc.w    $C87E        ; $01205E
        dc.w    $227B        ; $012060
        dc.w    $0004        ; $012062
        dc.w    $4ED1        ; $012064
        dc.w    $0089        ; $012066
        dc.w    $2084        ; $012068
        dc.w    $0089        ; $01206A
        dc.w    $2224        ; $01206C
        dc.w    $0089        ; $01206E
        dc.w    $250C        ; $012070
        dc.w    $4EBA        ; $012072
        dc.w    $9610        ; $012074
        dc.w    $0838        ; $012076
        dc.w    $0006        ; $012078
        dc.w    $C80E        ; $01207A
        dc.w    $6604        ; $01207C
        dc.w    $5878        ; $01207E
        dc.w    $C87E        ; $012080
        dc.w    $4E75        ; $012082
        dc.w    $4240        ; $012084
        dc.w    $4EBA        ; $012086
        dc.w    $C4A4        ; $012088
        dc.w    $207C        ; $01208A
        dc.w    $0601        ; $01208C
        dc.w    $8000        ; $01208E
        dc.w    $227C        ; $012090
        dc.w    $0400        ; $012092
        dc.w    $4C74        ; $012094
        dc.w    $303C        ; $012096
        dc.w    $0058        ; $012098
        dc.w    $323C        ; $01209A
        dc.w    $0010        ; $01209C
        dc.w    $4EBA        ; $01209E
        dc.w    $C2BA        ; $0120A0
        dc.w    $207C        ; $0120A2
        dc.w    $0601        ; $0120A4
        dc.w    $8900        ; $0120A6
        dc.w    $227C        ; $0120A8
        dc.w    $0401        ; $0120AA
        dc.w    $9010        ; $0120AC
        dc.w    $303C        ; $0120AE
        dc.w    $0120        ; $0120B0
        dc.w    $323C        ; $0120B2
        dc.w    $0010        ; $0120B4
        dc.w    $4EBA        ; $0120B6
        dc.w    $C2A2        ; $0120B8
        dc.w    $207C        ; $0120BA
        dc.w    $0601        ; $0120BC
        dc.w    $9B00        ; $0120BE
        dc.w    $227C        ; $0120C0
        dc.w    $0401        ; $0120C2
        dc.w    $C010        ; $0120C4
        dc.w    $303C        ; $0120C6
        dc.w    $0120        ; $0120C8
        dc.w    $323C        ; $0120CA
        dc.w    $0010        ; $0120CC
        dc.w    $4EBA        ; $0120CE
        dc.w    $C28A        ; $0120D0
        dc.w    $7000        ; $0120D2
        dc.w    $4A38        ; $0120D4
        dc.w    $A01A        ; $0120D6
        dc.w    $660A        ; $0120D8
        dc.w    $1038        ; $0120DA
        dc.w    $A019        ; $0120DC
        dc.w    $343C        ; $0120DE
        dc.w    $0010        ; $0120E0
        dc.w    $6008        ; $0120E2
        dc.w    $1038        ; $0120E4
        dc.w    $A01B        ; $0120E6
        dc.w    $343C        ; $0120E8
        dc.w    $FFC0        ; $0120EA
        dc.w    $1600        ; $0120EC
        dc.w    $43F9        ; $0120EE
        dc.w    $0089        ; $0120F0
        dc.w    $21FA        ; $0120F2
        dc.w    $D040        ; $0120F4
        dc.w    $D040        ; $0120F6
        dc.w    $2071        ; $0120F8
        dc.w    $0000        ; $0120FA
        dc.w    $303C        ; $0120FC
        dc.w    $0061        ; $0120FE
        dc.w    $4A03        ; $012100
        dc.w    $6604        ; $012102
        dc.w    $303C        ; $012104
        dc.w    $0060        ; $012106
        dc.w    $323C        ; $012108
        dc.w    $0010        ; $01210A
        dc.w    $4A39        ; $01210C
        dc.w    $00A1        ; $01210E
        dc.w    $5120        ; $012110
        dc.w    $66F8        ; $012112
        dc.w    $4EBA        ; $012114
        dc.w    $C29E        ; $012116
        dc.w    $7000        ; $012118
        dc.w    $4A38        ; $01211A
        dc.w    $A01A        ; $01211C
        dc.w    $670A        ; $01211E
        dc.w    $1038        ; $012120
        dc.w    $A019        ; $012122
        dc.w    $343C        ; $012124
        dc.w    $0010        ; $012126
        dc.w    $6008        ; $012128
        dc.w    $1038        ; $01212A
        dc.w    $A01C        ; $01212C
        dc.w    $343C        ; $01212E
        dc.w    $FFC0        ; $012130
        dc.w    $43F9        ; $012132
        dc.w    $0089        ; $012134
        dc.w    $2206        ; $012136
        dc.w    $D040        ; $012138
        dc.w    $3200        ; $01213A
        dc.w    $D040        ; $01213C
        dc.w    $D041        ; $01213E
        dc.w    $2071        ; $012140
        dc.w    $0000        ; $012142
        dc.w    $3031        ; $012144
        dc.w    $0004        ; $012146
        dc.w    $323C        ; $012148
        dc.w    $0010        ; $01214A
        dc.w    $4A39        ; $01214C
        dc.w    $00A1        ; $01214E
        dc.w    $5120        ; $012150
        dc.w    $66F8        ; $012152
        dc.w    $4EBA        ; $012154
        dc.w    $C25E        ; $012156
        dc.w    $4A39        ; $012158
        dc.w    $00A1        ; $01215A
        dc.w    $5120        ; $01215C
        dc.w    $66F8        ; $01215E
        dc.w    $33FC        ; $012160
        dc.w    $0101        ; $012162
        dc.w    $00A1        ; $012164
        dc.w    $512C        ; $012166
        dc.w    $33FC        ; $012168
        dc.w    $4000        ; $01216A
        dc.w    $00A1        ; $01216C
        dc.w    $5128        ; $01216E
        dc.w    $13FC        ; $012170
        dc.w    $002C        ; $012172
        dc.w    $00A1        ; $012174
        dc.w    $5121        ; $012176
        dc.w    $13FC        ; $012178
        dc.w    $0001        ; $01217A
        dc.w    $00A1        ; $01217C
        dc.w    $5120        ; $01217E
        dc.w    $4A39        ; $012180
        dc.w    $00A1        ; $012182
        dc.w    $512C        ; $012184
        dc.w    $66F8        ; $012186
        dc.w    $33FC        ; $012188
        dc.w    $0078        ; $01218A
        dc.w    $00A1        ; $01218C
        dc.w    $5128        ; $01218E
        dc.w    $33FC        ; $012190
        dc.w    $0101        ; $012192
        dc.w    $00A1        ; $012194
        dc.w    $512C        ; $012196
        dc.w    $7000        ; $012198
        dc.w    $1038        ; $01219A
        dc.w    $A019        ; $01219C
        dc.w    $4A38        ; $01219E
        dc.w    $A01A        ; $0121A0
        dc.w    $6704        ; $0121A2
        dc.w    $1038        ; $0121A4
        dc.w    $A01B        ; $0121A6
        dc.w    $21C0        ; $0121A8
        dc.w    $A01E        ; $0121AA
        dc.w    $7000        ; $0121AC
        dc.w    $1038        ; $0121AE
        dc.w    $A019        ; $0121B0
        dc.w    $4A38        ; $0121B2
        dc.w    $A01A        ; $0121B4
        dc.w    $6604        ; $0121B6
        dc.w    $1038        ; $0121B8
        dc.w    $A01C        ; $0121BA
        dc.w    $21C0        ; $0121BC
        dc.w    $A022        ; $0121BE
        dc.w    $207C        ; $0121C0
        dc.w    $0601        ; $0121C2
        dc.w    $BE00        ; $0121C4
        dc.w    $7200        ; $0121C6
        dc.w    $3038        ; $0121C8
        dc.w    $A02C        ; $0121CA
        dc.w    $670E        ; $0121CC
        dc.w    $5340        ; $0121CE
        dc.w    $0681        ; $0121D0
        dc.w    $0000        ; $0121D2
        dc.w    $0280        ; $0121D4
        dc.w    $51C8        ; $0121D6
        dc.w    $FFF8        ; $0121D8
        dc.w    $D1C1        ; $0121DA
        dc.w    $2278        ; $0121DC
        dc.w    $A034        ; $0121DE
        dc.w    $303C        ; $0121E0
        dc.w    $0028        ; $0121E2
        dc.w    $323C        ; $0121E4
        dc.w    $0060        ; $0121E6
        dc.w    $4EBA        ; $0121E8
        dc.w    $C170        ; $0121EA
        dc.w    $5878        ; $0121EC
        dc.w    $C87E        ; $0121EE
        dc.w    $33FC        ; $0121F0
        dc.w    $0020        ; $0121F2
        dc.w    $00FF        ; $0121F4
        dc.w    $0008        ; $0121F6
        dc.w    $4E75        ; $0121F8
        dc.w    $0401        ; $0121FA
        dc.w    $9010        ; $0121FC
        dc.w    $0401        ; $0121FE
